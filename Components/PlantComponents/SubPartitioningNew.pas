...
published
  ...
  Property Ex_EXLFW    : TExternV read EXLFW write EXLFW;
  ...
  Property Ex_v        : TExternV read v write v;
...
begin
  ...
  // Parameters
  ParCreate('G1_', '[-]',6G1_);
  ParCreate('G2_', '[-]',3.3G2_);
  ParCreate('G3_', '[mg/day]',4.5G3_);
  ParCreate('Plants', '[plants/m2]',350Plants);

         // External Variable
  ExternVCreate('AWR', '',statefield, AWR);
  ExternVCreate('CARBO', '',statefield, CARBO);
  ExternVCreate('DTT', '',statefield, DTT);
  ExternVCreate('EXLFW', '',statefield, EXLFW);
...
begin
   ...
   If  ISTAGE.v<2 then  CUMPH.c :=   TI.v   else  CUMPH.c :=   0  ;
   GPPend.v :=  0;
   GRNWT.v :=  GROGRN.v;
   If  (PLA.v>0)and(ISTAGE.v<6)and(ISTAGE.v>=1) then  LFWT.c :=   GROLF.v-SENL.v   else  LFWT.c :=   0  ;
   RTWT.c :=   0.6*GRORT.v-SENRT.v  ;
   If  (ISTAGE.v>=0.99)and(ISTAGE.v<2)and(SEEDRV.v>0) then  SEEDRV.c :=   CARBO.v-GROLF.v-GRORT.v   else  SEEDRV.c :=   0  ;
   If  -dlfwt.v>0 then  SENLF.c :=   DLFWT.v   else  SENLF.c :=   0  ;
   STMWT.v :=  GROSTM.v-SENSTM.v;
   SWMIN.v :=  0;
   ...
   If  (ISTAGE.v>=5)and(ISTAGE.v<6)and(CARBO.v*PTF.v<RGFILL.v*GPP.v*G2.v*0.001)and(STMWT.v<SWMIN.v) then  GROGRN.v :=   CARBO.v*PTF.v   else  If  (ISTAGE.v>=5)and(ISTAGE.v<6) then  GROGRN.v :=   RGFILL.v*GPP.v*G2.v*0.001   else  GROGRN.v :=   0  ;
   If  (ISTAGE.v>=1)and(ISTAGE.v<2) then  GROLF.v :=   min((PLAGMS.v*(0.3+0.7*TILN.v))/AWR.v, CARBO.v*0.65+SEEDRV.v)   else  If  (ISTAGE.v>=2)and(ISTAGE.v<3) then  GROLF.v :=   CARBO.v-GRORT.v-GROSTM.v   else  GROLF.v :=   0  ;
   If  (ISTAGE.v>=2)and(ISTAGE.v<6) then  GRORT.v :=   (1-PTF.v)*CARBO.v   else  If  (ISTAGE.v>=1)and(ISTAGE.v<2) then  GRORT.v :=   max(CARBO.v*0.35, CARBO.v-GROLF.v)   else  GRORT.v :=   0  ;
   If  (ISTAGE.v>=2)and(ISTAGE.v<3) then  GROSTM.v :=   (.15+0.12*SUMDTT2.v/PHINT.v)*CARBO.v*PTF.v   else  If  (ISTAGE.v>=3)and(ISTAGE.v<5) then  GROSTM.v :=   CARBO.v*PTF.v   else  If  (ISTAGE.v>=5)and(ISTAGE.v<6) then  GROSTM.v :=   CARBO.v*PTF.v-GROGRN.v   else  GROSTM.v :=   0  ;
   LFWT_m2.v :=  LFWT.v*Plants.v;
   If  (ISTAGE.v<6)and (ISTAGE.v>=5) and (stmwt.v>0.) then  PTF.v :=   min(1,SWMIN.v/STMWT.v*0.35+0.65)   else  If  (ISTAGE.v<5)and (ISTAGE.v>=4) then  PTF.v :=   0.8+0.1*min(swdf1.v,ndef2.v)   else  If  (ISTAGE.v<4)and (ISTAGE.v>=3) then  PTF.v :=   0.75+0.1*min(swdf1.v,ndef2.v)   else  If  (ISTAGE.v<3)and (ISTAGE.v>=2) then  PTF.v :=   0.70+0.1*min(swdf1.v,ndef2.v)   else  If  (ISTAGE.v<2)and(ISTAGE.v>=1) then  PTF.v :=   0.5   else  PTF.v :=   0.0  ;
   If  (TEMPM.v<10)and(ISTAGE.v>=5)and(ISTAGE.v<6) then  RGFILL.v :=   0.065*TEMPM.v   else  If  (ISTAGE.v>=5)and(ISTAGE.v<6) then  RGFILL.v :=   0.65+(0.0787-0.00328*(tempm.vx-tempm.vn))*(tempm.v-10.0)^0.8   else  RGFILL.v :=   0  ;
   RTWT_m2.v :=  RTWT.v*plants.v;
   If  (PLA.v>0)and(ISTAGE.v<6)and(ISTAGE.v>=1) then  SENL.v :=   LFWT.v*0.000267*DTT.v*(1.-SENLA.v/PLA.v)   else  SENL.v :=   0  ;
   SENRT.v :=  0.005*RTWT.v;
   If  (ISTAGE.v>=1)and(ISTAGE.v<6) then  SENSTM.v :=   STMWT.v*0.000267*DTT.v   else  SENSTM.v :=   0  ;
   STMWT_m2.v :=  STMWT.v*Plants.v;
   If  gpsm.v>0 then  TKM.v :=   GRNWT_m2.v/gpsm.v*1000   else  TKM.v :=   0  ;
...
begin
  ...
  inherited  integrate;
  If (ISTAGE>=1) then begin
    // message("Init S1","Message");;
    LFWT=0.00034;;
    SEEDRV=0.012;;
    // TOPWT=SEEDRV*0.5;;
    // RTWT=SEEDRV*0.5;;
  ...
  If (ISTAGE>=4) then begin
    SWMIN=STMWT;;
  end;
  If (ISTAGE>=5) then begin
    grnwt=0.0035*gpp;;
    gppend=gpp;;
    // Message("Init S5", "");;
  end;
