...
public
  ...
  f_FertSens : TPar;  // factor to change in sensitivity analysis to change fertilizer rate
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          InputDates.AddObject(Datum, TAction.create(Add,
            FindExVar(Actions[j]), V, 0, 0));
        end;
        if (C > 0) and (N > 0) then
        begin
...
begin
  inherited;
  if GlobTime.v >= NextDate then begin
    InputDates.Find(IntToStr(NextDate),i);
    while StrToInt(InputDates[i]) <= GlobTime.V do begin
      case TAction(InputDates.Objects[i]).A of
        Residues: begin
                    if Assigned(fSoilMinMod) then fSoilMinMod.AddResidues(TAction(InputDates.Objects[i]).C * f_FertSens.v,TAction(InputDates.Objects[i]).N * f_FertSens.v);
                  end;
        Add:      begin
      ...
      Inc(i);
      if i >= InputDates.count then begin
        NextDate := 100000;
        exit;
      end;
    end;
...
begin
  result := '0';
  if s <> '' then begin
    if Pos('.',s) > 0 then result := IntToStr(round(StrToDate(s)))
                      else if StrToIntDef(s,0)=StrToIntDef(s,1) then result := s;
  end;
end;
...
begin
  ...
  for i := 0 to ExtVars.Count - 1 do
    if (ExtVars[i] = s) or ((Pos('.',ExtVars[i])>0)and(Copy(ExtVars[i],Pos('.',ExtVars[i])+1,length(ExtVars[i])-Pos('.',ExtVars[i]))=VName)) then begin
