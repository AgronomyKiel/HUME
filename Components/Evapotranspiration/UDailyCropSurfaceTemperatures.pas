...
type
 ...
 measTcMax:             TExternV;  //measured maximum canopy temperature from weather file to replace MaxCanopyTemp if Opt_Tc_source is set to fromWeatherFile
 measTcMean:            TExternV;  //measured mean canopy temperature from weather file to replace MeanCanopyTemp if Opt_Tc_source is set to fromWeatherFile
 measTcMin:             TExternV;  //measured minimum canopy temperature from weather file to replace MinCanopyTemp if Opt_Tc_source is set to fromWeatherFile
 HeatRing_IO:           TExternV;  // O/I  signal that reflects if maesTc[...] are maniupulated with T-FACE technique
 ...
 property Ex_relCO2TransDiff:  TExternV  read relCO2TransDiff write relCO2TransDiff;
...
*}
  ...
  for i := 1 to 24 do begin
    ...
    if LAI>0 then begin
     TDiffA45_arr[i]:=
     pIntT45corr.v +
     pCT_0T45corr.v               * TC_Hourly[i].v +
     pdayTMPMT45corr.v            * TMPM.v  +
     pTDiffMeanT45corr.v          * TDiffMean.v +
     pTDiffMaxT45corr.v           * TDiffMax.v +
     pln_LAIT45corr.v             * ln(LAI) +
     pSunAngleT45corr.v           * Hourly_Elevation[i].v +
     pRad_IntT45corr.v            * Rad_Int.v +
     pTDiffMean_lnLAIT45corr.v    *  TDiffMean.v          * ln(LAI)+
...
h_temp:=Tmax*(p1-p2*sin((x*pi)/12+p3)
       +0.11*sin((x*2*pi)/12+p3))
       +Tmin*(1-(p1-p2*sin((x*pi)/12+p3)
       +0.11*sin((x*2*pi)/12+p3)));
result:=h_temp;
...
begin
  ...
  ExternVCreate('CO2TransDiff',  '[mm/d]', statefield, CO2TransDiff,'CO2 induced reduction of pot_trans');
  ExternVCreate('relCO2TransDiff',  '[-]', statefield, relCO2TransDiff,'rel. CO2 induced reduction of pot_trans');
  ...
  ParCreate('pTDiffMean_lnLAIT45corr', '[-]',  -2.327e-01, pTDiffMean_lnLAIT45corr);
  ParCreate('pTDiffMean_SunAngleT45corr', '[-]',  -4.626e-03, pTDiffMean_SunAngleT45corr);
  ...
  ParCreate('pSunAngle_Rad_IntT45corr', '[-]',  -3.233e-05, pSunAngle_Rad_IntT45corr);

  ParCreate('CO2IncreaseTcmin', '[°C mm^-1 d^-1]',  0.04282732, CO2IncreaseTcmin,
              'changes canopy minimum canopy temperature as function of relCO2TransDiff');        //derived from Braunschweig FACE trial 2013/2014 & 2014/2015 (nadir viewing angle 45°)
  ParCreate('CO2IncreaseTcmean', '[°C mm^-1 d^-1]',  -1.41535761, CO2IncreaseTcmean,
              'changes canopy mean canopy temperature as function of relCO2TransDiff');          //derived from Braunschweig FACE trial 2013/2014 & 2014/2015  (nadir viewing angle 45°)
  ParCreate('CO2IncreaseTcmax', '[°C mm^-1 d^-1]',  -2.16768115, CO2IncreaseTcmax,
              'changes canopy maximum canopy temperature as function of relCO2TransDiff');          //derived from Braunschweig FACE trial 2013/2014 & 2014/2015

  ParCreate('lon', '[decimal degree]', 15, lon,'local longitude');
  ParCreate('lonCTZ', '[decimal degree]', 15, lonCTZ,'longitude of the time zone');
  ...
  ParCreate('fWMeanTpheno', '[-]', 0.681876, fWMeanTpheno, 'weighting factor to include mean. canopy temperature in Development');
  ParCreate('fWMeanTDrymatter', '[-]', 3.38952, fWMeanTDrymatter, 'weighting factor to include max. canopy temperature in Dry Matter Production');
  ParCreate('fWMeanTPartitioning', '[-]', 1.9167, fWMeanTPartitioning, 'weighting factor to include mean. canopy temperature in Partitioning');
  ...
  begin
    ...
    if i<11 then
      VarCreate('TC_Hourly_45__' + IntToStr(i-1), '[°C]', 0, true, TC_Hourly_45[i])
    else
      VarCreate('TC_Hourly_45_' + IntToStr(i-1), '[°C]', 0, true, TC_Hourly_45[i]);

    if i<11 then
      VarCreate('Hourly_Elevation__' + IntToStr(i-1), '[°]', 0, true, Hourly_Elevation[i])
    else
      VarCreate('Hourly__Elevation' + IntToStr(i-1), '[°]', 0, true, Hourly_Elevation[i]);
  end;
  ...
  VarCreate('TmaxCO2Change', '[°C]', 0,true, TmaxCO2Change,'absolute change of maximum canopy temperature due to CO2');
  VarCreate('TmeanCO2Change', '[°C]', 0,true, TmeanCO2Change,'absolute change of mean canopy temperature due to CO2');
  VarCreate('TminCO2Change', '[°C]', 0,true, TminCO2Change,'absolute change of minimum canopy temperature due to CO2');
    VarCreate('PseudoMeanTemp', '[°C]', 0, true, PseudoMeanTemp, 'hourly averaged temp. of the sinus distribution');
...
begin
  inherited;
  if (Opt_Tc_source.option = 'fromweatherfile') or (Opt_Tc_source.option = 'manipulated_with_heatring') then begin
...
begin
 ...
 if (EC.v>31) then
 begin
     if(LAI.v>0) and (pETP.v>0)then begin
  ...
  if Opt_Tc_source.Option = 'fromweatherfile' then begin
    if (measTcMax.v<>0) and (measTcMean.v <>0) and (measTcMin.v<>0) then begin
     MeanCanopyTemp.v:=measTcMean.v;
     MinCanopyTemp.v:= measTcMin.v;
     MaxCanopyTemp.v:= measTcMax.v
    end;
  end;
  if Opt_Tc_source.Option = 'manipulated_with_heatring' then begin
    if (measTcMax.v<>0) and (measTcMean.v <>0) and (measTcMin.v<>0) and (HeatRing_IO.v=1) then begin
     MeanCanopyTemp.v:=measTcMean.v;
     MinCanopyTemp.v:= measTcMin.v;
     MaxCanopyTemp.v:= measTcMax.v
    end;
