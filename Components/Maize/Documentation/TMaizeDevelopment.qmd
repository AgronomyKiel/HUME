...
Maize development is regulated by the effective temperature (\$T\_{eff}\$), which is calculated as the positive difference between the daily mean temperature and a defined base temperature (@tbl-Parameter) of 6 °C, following @Pages.1994 and @Bonhomme.1994.
...
Development rates between the individual stages are calculated as the ratio between effective temperature (\$T\_{eff}\$) and the corresponding temperature sums (growing degree days, GDD). Specifically, development rates are defined for the phases between sowing and emergence (@eq-DevRateS0), emergence and tassel initiation (@eq-DevRateS1), tassel initiation and silking (@eq-DevRateS2), and silking and grain filling (@eq-DevRateS3). The corresponding temperature sums are defined by the parameters GDDemer, GDDtasini, GDDS2, and GDDS3 (seet Table @tbl-parameter).
...
#| message: false
#| warning: false
#| tbl-cap: Parameters of the TSubDevelopmentMaize submodel 
...
| $GDD_{emer}$ {#parm-GDDemer} | 100 | °C d | Temperature sum from sowing until emergence | @Bignon.1990 |
