...
public
  ...
  GDD6Stage3 : TPar;   ///  Temperature sum stage 3 : from silking till grain filling
  GDD6total : TPar;   ///  Temperature sum  from sowing till maturity
  GDD6tasini : TPar; ///Temperature sum from emergence till tassel initiation
  GDD6Stage2 : TPar; ///Temperature sum from tassel initiation till silking
...
published
  ...
  Property Par_GDD6tasini : TPar read GDD6tasini write GDD6tasini;
  Property Par_GDD6Stage2 : TPar read GDD6Stage2 write GDD6Stage2;
...
begin
  ...
  // Parameters; GDD's calculated according to Bignon et al 1990; GDD for middle early cultivar

  ParCreate('GDD6Emergence', '[°C*d]',100,GDD6Emergence, 'Temperature sum sowing to emergence');
  ParCreate('GDD6Silking', '[°C*d]',830,GDD6Silking, 'Temperature sum silking');
  ParCreate('GDD6Stage3', '[°C*d]',200,GDD6Stage3, 'Temperature sum stage 3 silking till grain filling');
  ParCreate('GDD6total', '[°C*d]',1890,GDD6total, 'Temperature sum from sowing till maturity');
  ParCreate('GDD6tasini', '[°C*d]',320,GDD6tasini, 'Temperature sum from emergence till tassel initiation');
  ParCreate('GDD6Stage2', '[°C*d]',510,GDD6Stage2, 'Temperature sum tassel initiation till silking');
...
begin
  ...
  // If  XSTAGE.v>=0 then  GDD8.c :=Teff8.v else  GDD8.c :=0;
   ...
   If  (XSTAGE.v>=1) and (XSTAGE.v<=2)
       ...
       then DevRateS1.v := Teff6.v/GDD6tasini.v
       else DevRateS1.v :=0;
   If  (XSTAGE.v>=2) and (XSTAGE.v<=3)
      // then  DevRateS2.v :=Teff6.v/TSum_LeafLagphase.v
       then DevRateS2.v:=Teff6.v/GDD6Stage2.v
       else  DevRateS2.v :=0;
   If  (XSTAGE.v>=3) and (XSTAGE.v<=4)
        then DevRateS3.v :=Teff6.v/GDD6Stage3.v
        else  DevRateS3.v :=0;
   If  (XSTAGE.v>=4) and (XSTAGE.v<=5)
        then DevRateS4.v :=Teff6.v/(GDD6total.v-GDD6Stage3.v-GDD6Emergence.v-GDD6Silking.v)
        else DevRateS4.v :=0;
  ...
  If  XSTAGE.v>=1
   ...
   If // (TSum_leafLagphase.v+GDD6_from_emergence.v<=GDD6Silking.v)and(xstage.v>=1)and(Teff6.v>0)
      (TSum_leafLagphase.v<=GDD6Silking.v)and(xstage.v>=1)and(Teff6.v>0)
       then  TSum_leafLagphase.c :=(Teff6.v/Plastochron.v-TI.v)*Phyllochron.v  //noch benötigte TemSum zur Ausbildung (hier als Rate)
       ///der angelegten Blätter (zu jedem Zeitpunkt)
   ...
   IsAfterSowing := false;
     ...
     //(XSTAGE.v>= 3) then
     Sen_fact.v:= exp(XSTAGE.v*sen_parexp.v)*sen_par.v;     // neue Berechnung des sen-Faktors (holzhauser)
