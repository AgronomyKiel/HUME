...
begin
 ...
 // VarCreate('DMShoot', '',0, true, DMShoot);
  ...
  if (WithRoots = true) and (soillayermod <> nil) then begin
    for i := 1 to tlayeredsoil(soillayermod).p_NComp do begin
      ...
      ExternVCreate('WLD_'+inttostr(i),'[cm/cm3]',StateField, exWLD_arr[i]);
      VarCreate ('WLD'+inttostr(i),'',0,true, WLD[i]);
    end;
  end;
...
var
  i:integer;
begin
   ShootGR.v :=TotTMgRate.v;
   ExtPAR_varLAI.v := ExtCoeffPAR;

    If  (XSTAGE.v>=1)and(XSTAGE.v<2)and(SEEDRV.v>0) then
    begin
      SEEDRV.c   :=max(-SEEDRV.v,-k_SEEDRV.v*Tempfact.v);
      ShootGR.v := ShootGR.v-SEEDRV.c;       // Addition of seed reserves to ShootGR
    end
  else  SEEDRV.c :=   0;
   If  (XStage.v>=1) and (CumPH.v<TLNO.v-2)  // (CumPH.v<TLNO.v-2) entspricht GDD6_from_emergence < GDDsilking-2.46*Phyllochron
       then  If DMleaf.v > 0 then fLEAF.v :=   1/(1+exp(flb.v)*fla.v*power(DMleaf.v,(fla.v-1))) else  fleaf.v:= 1
       else  fLEAF.v :=   0  ;
  ...
  // end
  // else fcob.v :=0;
   If DMStem.v <= 0.25576*DMShoot.v then fCob.v:= min(1,fCob.v);   // 0.25576 resultieren
   ...
   If (LAI.v>0) or (SeedRV.c<>0) then begin
      If(fSLA_ = SLAfGAI) then begin
        ...
        SLAstem.v := min(SLAstemini.v,SLAs_a.v*power(LAIstem.v,SLAs_b.v)+SLAs_a.v*SLAs_b.v*power(LAIstem.v,SLAs_b.v-1)*LAIstem.v);
   ...
   for i := 1 to tlayeredsoil(soillayermod).p_NComp do WLD[i].v := ExWLD_arr[i].v;
...
begin
  ...
  LAItotal.v := LAIleaf.v+LAIstem.v;
  CropHeight.v := LAItotal.v*fCropHeight.v;
  If DMleaf.v>0 then
