...
Growing grains represent a strong sink for nitrogen (N) and trigger N remobilization from the vegetative organs, which decreases canopy photosynthesis and accelerates leaf senescence. The N dynamics for specific leaf-layers, stem and grain during grain-filling is simulated according to the process based approach of [@Bertheloot2008b]. According to [@Meinke1988] a SLN threshold value SLNcrit [g N^.^m^-2^] is used for the calculation of leaf senescence due to N remobilization.
The critical leaf N concentration of a specific leaf-layer cNcrit[i] [%] is calculated as follows:
...
The total amount of N in leaf layers and stem is divided into a structural fraction (strucNl[i], strucNs) and photosynthetic fraction (phNl[i], phNs). According to [@Bertheloot2008b], the mobile N pool (mobNtot) is decomposed into a leaf and a stem fraction (mobNl, mobNs).
...
$${}^{ph}{{N}_{l[i]}}={{N}_{l[i]}}-{{W}_{l[i]}}\cdot c{{N}_{struc}}$$	{#eq-LA36}

and

$${}^{ph}{{N}_{s}}={{N}_{stem}}-{{W}_{stem}}\cdot c{{N}_{struc}}$$	{#eq-LA37}
...
$${}^{mob}{{N}_{s}}={}^{mob}{{N}_{tot}}\cdot \frac{{{N}_{stem}}}{{{N}_{stem}}+{{N}_{leaf}}}$$	{#eq-LA38}

respectively

$${}^{mob}{{N}_{l}}={}^{mob}{{N}_{tot}}-{}^{mob}{{N}_{S}}$$ {#eq-LA39}


##	Dynamics of photosynthetic N during grain filling period

The physiological senescence rate during grain filling is driven by the N translocation from the vegetative tissue to the grains. Once leaf area is senescent, the difference between structural and current N content in senescent leaf tissue (transN[i]) is stored to the ${}^{mob}{{N}_{l}}$ pool, whereas the ${}^{ph}{{N}_{l[i]}}$pool is reduced proportionally. Thus the concentration of photosynthetic N in the layer is not enhanced by senescence process. On this issue the implementation differs from the original approach, which assumes ${}^{ph}{{N}_{l[i]}}$ as being unaffected by senescence. As a consequence senescence increases leaf N concentration. Generally, the differences between the methods are small (not shown) since transN[i] enhances the mobile N pool and thereby the synthesis of photosynthetic leaf N (@eq-LA47).
According to [@Bertheloot2008b] the dynamic of phNL[i] is modelled as the difference between a synthesis rate (phS[i] (t), [kg^.^°Cd^-1^]) and degeneration rate (phD[i] (t), [kg^.^°Cd^-1^]):

$$\frac{{{d}^{ph}}{{N}_{l[i]}}}{dt}={{[}^{ph}}{{S}_{[i]}}(t){{-}^{ph}}{{D}_{[i]}}(t)]\times {{10}^{3}}-{}^{trans}{{N}_{[i]}}(t)$$ 	{#eq-LA45}
...
$${ph}{{D}_{[i]}}(t)=\delta \cdot {}^{ph}{{N}_{[i]}}\times {{10}^{-3}},$$	{#eq-LA46}

where δ [°Cd^-1^] is the relative rate of phN[i] degeneration. 

The synthesis rate is modelled as a double Michaelis-Menten function:

$${ph}{{S}_{[i]}}(t)=\sigma \cdot {{W}_{l}}_{[i]}(t)\times {{10}^{-3}}\cdot \frac{{}^{mob}{{N}_{c}}(t)}{{}^{mob}{{N}_{c}}(t)+k1}\cdot \frac{PAR}{PA{{R}_{[i]}}+k2},$$
{#eq-LA47}
...
$${}^{mob}{{N}_{c}}(t)=\frac{{}^{^{mob}}{{N}_{l}}(t)+{}^{^{mob}}{{N}_{s}}(t)}{{{W}_{leaf}}(t)+{{W}_{stem}}(t)}$$	{#eq-LA48}

According to [@Bertheloot2008b], the amount of intercepted PAR[i] was calculated as a function of cumulative leaf area index above the respective lamina F[i]:

$$PA{{R}_{[i]}}={{k}_{PAR}}\cdot PA{{R}_{0}}\cdot pTrans\cdot \exp (-{{k}_{PAR}}\cdot {{F}_{[i]}})$$	{#eq-LA49}
...
$$r=\frac{^{mob}{{N}_{l}}}{\sum\limits_{i}{^{_{ph}}{{N}_{l[i]}}}}$$	{#eq-LA50}

the change rate of phNs therefore is

$$\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{s}}}}{\text{dt}}\text{= min(0,}{}^{\text{ph}}{{\text{N}}_{\text{s}}}(t)\times \text{r(t) - }{}^{\text{mob}}{{N}_{\text{s}}}(t))$$	{#eq-LA51}

In contrast to the original model, the transfer of mobile N to the grain N pool starts with stem fraction (mobNs). The change rate of mobNs is calculated as the difference between stem N release and grain N uptake (@eq-LA44) as follows:

$$\frac{\text{d}{}^{\text{mob}}{{\text{N}}_{\text{s}}}}{\text{dt}}\text{= max(-}{}^{\text{mob}}{{\text{N}}_{\text{s}}}\text{(t),-}\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{s}}}}{\text{dt}}\text{-}\frac{\text{d}{{\text{N}}_{\text{g}}}}{\text{dt}})$$	{#eq-52}


Therefore a larger amount of N remains at the mobNl pool (@eq-LA38), compared to mobNs whenever the amount of mobile N exceeds the N demand of the grains: 

$$\frac{\text{d}{}^{\text{mob}}{{\text{N}}_{\text{l}}}}{\text{dt}}\text{= }\left\{ \begin{align}
  & \max (-{}^{mob}{{N}_{l}}(t),-\sum\limits_{\text{i}}{\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{l}}}}{\text{dt}}}+\sum{{}^{trans}{{N}_{[i]}}(t)}) &{{|}^{\text{mob}}}{{\text{N}}_{\text{s}}}(t)\ge (\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{s}}}}{\text{dt}}+\frac{\text{d}{{\text{N}}_{\text{grain}}}}{\text{dt}})\\ 
 & \max (-{}^{mob}{{N}_{l}}(t),-\sum\limits_{\text{i}}{\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{l}}}}{\text{dt}}}-(\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{s}}}}{\text{dt}}+\frac{\text{d}{{\text{N}}_{\text{grain}}}}{\text{dt}}) (\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{s}}}}{\text{dt}}+\frac{\text{d}{{\text{N}}_{\text{grain}}}}{\text{dt}})  
 & {{|}^{\text{mob}}}{{\text{N}}_{\text{s}}}(t)<(\frac{\text{d}{}^{\text{ph}}{{\text{N}}_{\text{s}}}}{\text{dt}}+\frac{\text{d}{{\text{N}}_{\text{grain}}}}{\text{dt}})\\ 
 & \mathop{{}}^{{}}\mathop{{}}^{{}}+{}^{mob}{{N}_{s}}(t)+\sum{{}^{trans}{{N}_{[i]}}(t)}). \\ 
