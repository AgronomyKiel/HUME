...
unit UHumeWheatPartitioning_iter_try;
...
type
  ...
  public
    ...
    NcStemMin: TPar; //< niedrigste stem N-concentration (%)
...
begin
  ...
  begin
    ...
    iter := 0;
    iterLeafwt := LFWT_pl.v*Plants.v + (1-fStem_)*Assiflow.v*GlobTime.c;
    iterVegDM := LFWT_pl.v*Plants.v+STMWT_pl.v*Plants.v+Assiflow.v*GlobTime.c;
    delta := fStem_;
    repeat
      iterStemwt := exp(h.v)*power(iterLeafwt, g.v);
      tmp := iterLeafwt;
      iterLeafwt := iterVegDM-iterStemwt;
      delta := abs(1-iterLeafwt/tmp);
      inc(iter);
    until (delta<0.0001) or (iter>20);
    STMWT_pl.c := (iterStemwt-STMWT_m2.v)/Plants.v;
...
begin
  ...
  ParCreate('Ini_SEEDRV', '[-]', 0.05, Ini_SEEDRV, 'initial seed nitrogen reserve');
