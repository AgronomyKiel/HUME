...
interface  // bestimmt, was in der Unit von au�en zug�nglich ist
...
//die auf dem Formular angeordnet sind, sowie die zum Formular geh�renden
//Prozeduren)
  ...
  public
    ...
    inL_MS      // initial leaf number on main stem
       : TState;
    inl_MS_xstage2 : TVar;// Anzahl nicht ausgebildeter Bl�tter zum Zeitpunkt Xstage 2
...
begin
  inherited createAll;//M�glichkeit, die Methoden des Vorg�ngermodells
...
// inl_MS_xstage2.v := max(0, inl_MS.v-{2-}nl_MS.v);     // two leaves will never emerge ...
  ...
  begin
     ...
     If EC.v < 37 then
       ...
       EC.c := min(2*tsuminc.v/Ph39_opt.v,40-EC.v);// 40-EC.v beschr�nkt das Entw.rate auf �ber 40 springt  min(tsuminc.v/Phint.v,40-EC.v);
