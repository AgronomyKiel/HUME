...
TTillerdevelopmentSimple = class(TSubmodel)
...
//  w1 : TVar;   // soil water deficit factor for tiller number increase
  ...
  TILSW : TState;   // weight of a single tiller [g/tiller]
             ...
             // Parameters
  G3 : TPar; //  Non-stressed dry weight of a single stem (excluding leaf blades and sheaths) and spike when elongation ceases (g).
...
//  Property Var_w1 : TVar read w1 write w1;
  ...
  Property Par_G3 : TPar read G3 write G3;
...
procedure TTillerdevelopmentSimple.createAll;
...
begin
  ...
  VarCreate('TC2', '',0, true, TC2, 'Rate of tiller formation based on the competition limitations of tillers per square meter');
...
//  VarCreate('w1', '',0, true, w1);
  ...
  // Parameters
  ParCreate('G3', 'g', 1.9 , G3, 'Non-stressed dry weight of a single stem (excluding leaf blades and sheaths) and spike when elongation ceases (g).');
...
procedure TTillerdevelopmentSimple.init(var GlobMod: TMod);
...
procedure TTillerdevelopmentSimple.CalcRates;
...
//   sn1.v :=  1-0*(1.4*NDEF3.v-0.4);
   ...
   If  (ISTAGE.v>=2)and(ISTAGE.v<4)
     then  TILLdeath.v :=   TILN.v*DTT.v*0.005*(1-RTSW.v)  //geändert Ratjen 15.08.2008
   else  TILLdeath.v :=   0  ;
...
procedure TTillerdevelopmentSimple.Integrate;
...
begin
  ...
  If (ISTAGE.v>=1) and (TILN.v<1) then begin
    TILN.v  := 1.;
  end;
...
procedure Register;
begin
  RegisterComponents('Ceres Wheat', [TTillerdevelopmentSimple]);
end;
