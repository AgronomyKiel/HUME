...
unit Development_Barley; // Kopfzeile
...
//Prozeduren)
  ...
  TDevelopment_Barley = class(TSubmodel)
  private
  ...
  public
    ...
    d30,     // Day when EC equals 29
    d37,
    d39,
    d41,     // Day when EC equals 50
    d43,
    d45,
    d47,
    d49,
...
procedure TDevelopment_Barley.createall;
begin
  ...
  // Vorgängermethoden übernommen werden
   ...
   VarCreate('phint', '[°Cd]', 0, true, phint);
   ...
   VarCreate('d30', '', 0, true, d30);
   VarCreate('d37', '', 0, true, d37);
   VarCreate('d39', '', 0, true, d39);
   VarCreate('d41', '', 0, true, d41);
   VarCreate('d43', '', 0, true, d43);
   VarCreate('d45', '', 0, true, d45);
   VarCreate('d47', '', 0, true, d47);
   VarCreate('d49', '', 0, true, d49);
   VarCreate('d50', '', 0, true, d50,'Day when EC equals 50');
   VarCreate('d51', '', 0, true, d51);
   VarCreate('d55', '', 0, true, d55);
   VarCreate('d59', '', 0, true, d59);
   VarCreate('d65', '', 0, true, d65);
   VarCreate('d69', '', 0, true, d69);
   VarCreate('d71', '', 0, true, d71);
   VarCreate('d73', '', 0, true, d73);
   VarCreate('d75', '', 0, true, d75);
   VarCreate('d77', '', 0, true, d77);
   VarCreate('d83', '', 0, true, d83);
   VarCreate('d85', '', 0, true, d85);
   ...
   Parcreate('p1d', '', 4 ,p1d);  //aktualisiert nach John-Manuskript 29.Jan.09
   Parcreate('p1v', '', 3.5  ,p1v);  //aktualisiert nach John-Manuskript 29.Jan.09
   ...
   ParCreate('p9', '°Cd', 135.0, p9);  //aktualisiert nach John-Manuskript 29.Jan.09
   ...
   ParCreate('Ph39', '-', 82.25, Ph39); //aktualisiert nach John-Manuskript 29.Jan.09
   ParCreate(' xstage_fin_leaf_prim', '-',1.78171 ,  xstage_fin_leaf_prim); // neu angepasst 17.01.
   ParCreate('dl_phint_int', '[°Cd]',188.957 ,  dl_phint_int,'phint daylength dependency (intercept)');
   ParCreate('dl_phint_inc', '[°Cd/d]',-9.389  ,  dl_phint_inc,'phint daylength dependency (slope)');
...
// comments initialisation
    ...
    d30.comment :=   'Day when EC equals 30';
...
procedure TDevelopment_Barley.init(var GlobMod: TMod);
...
procedure TDevelopment_Barley.CalcRates;

begin
// Variablen Prozeduren *************

  phint.v:= dl_phint_int.v + dl_phint_inc.v * daylengthp.v;
...
// inl_MS_xstage2.v := max(0, inl_MS.v-{2-}nl_MS.v);     // two leaves will never emerge ...
  ...
  begin
     ...
     //EC.c := zstage.v - EC.v;
       If EC.v < 37 then
      begin
         EC.c     := tsuminc.v/phint.v;   // EC stage change according to the inverse of the temperature sum between the appearance of two internodes
         xstage.c := tsuminc.v*(1/(inl_MS_xstage2.v*Phint.v+Ph39.v))//EC.c/(40-29);
      end
      else begin
         EC.c := min(2*tsuminc.v/Ph39.v,40-EC.v);// 40-EC.v beschränkt das Entw.rate auf über 40 springt  min(tsuminc.v/Phint.v,40-EC.v);
         xstage.c := tsuminc.v*(1/(inl_MS_xstage2.v*Phint.v+Ph39.v))//min(EC.c/(40-29), 3-Xstage.v);
      end ;
...
procedure TDevelopment_Barley.Integrate;
...
// non-iecnteger growth stage indicator ranging from zero to six
  ...
  if (d29.V <=0) and (ec.v >=29) then
    d29.v := dayssow.v;
  if (d30.V <=0) and (ec.v >=30) then
    d30.v := dayssow.v;

  if (d37.V <=0) and (ec.v >=37) then
    d37.v := dayssow.v;

  if (d39.V <=0) and (ec.v >=39) then
    d39.v := dayssow.v;

  if (d41.V <=0) and (ec.v >=41) then
    d41.v := dayssow.v;

  if (d43.V <=0) and (ec.v >=43) then
    d43.v := dayssow.v;

  if (d45.V <=0) and (ec.v >=45) then
    d45.v := dayssow.v;

  if (d47.V <=0) and (ec.v >=47) then
    d47.v := dayssow.v;

  if (d49.V <=0) and (ec.v >=49) then
    d49.v := dayssow.v;

  if (d50.V <=0) and (ec.v >=50) then
    d50.v := dayssow.v;

  if (d51.V <=0) and (ec.v >=51) then
    d51.v := dayssow.v;

  if (d55.V <=0) and (ec.v >=55) then
    d55.v := dayssow.v;

   if (d59.V <=0) and (ec.v >=59) then
    d59.v := dayssow.v;

  if (d65.V <=0) and (ec.v >=65) then
    d65.v := dayssow.v;

  if (d69.V <=0) and (ec.v >=69) then
    d69.v := dayssow.v;

  if (d71.V <=0) and (ec.v >=71) then
    d71.v := dayssow.v;

   if (d73.V <=0) and (ec.v >=73) then
    d73.v := dayssow.v;

  if (d75.V <=0) and (ec.v >=75) then
    d75.v := dayssow.v;

  if (d77.V <=0) and (ec.v >=77) then
    d77.v := dayssow.v;

  if (d83.V <=0) and (ec.v >=83) then
    d83.v := dayssow.v;

  if (d85.V <=0) and (ec.v >=85) then
    d85.v := dayssow.v;
...
procedure TDevelopment_Barley.AddDataValueToDataSeries;
...
procedure TDevelopment_Barley.AddSimValueToDataSeries;
...
procedure Register;
begin
  RegisterComponents('CERES Wheat', [TDevelopment_Barley]);
end;
