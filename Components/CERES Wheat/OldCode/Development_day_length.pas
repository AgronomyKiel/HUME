...
unit Development_day_length; // Kopfzeile
...
//Prozeduren)
  ...
  TDevelopment_dl = class(TSubmodel)
  private
...
procedure TDevelopment_dl.createall;
begin
  ...
  // Vorgängermethoden übernommen werden
   ...
   VarCreate('TSumInternode',    '[°Cd]', 0, true, TSumInternode, 'a function of phint and day length');
   VarCreate('ph39',    '[°Cd]', 0, true, ph39, 'a function of phint and day length');
...
procedure TDevelopment_dl.init(var GlobMod: TMod);
...
procedure TDevelopment_dl.CalcRates;

begin
// Variablen Prozeduren *************

  TSumInternode.v:= phint.v + fdl.v*daylengthp.v;
  ph39.v:=TSumInternode.v;
...
// inl_MS_xstage2.v := max(0, inl_MS.v-{2-}nl_MS.v);     // two leaves will never emerge ...
  ...
  begin
     ...
     //EC.c := zstage.v - EC.v;
       If EC.v < 37 then
      begin
         EC.c     := tsuminc.v/TSumInternode.v;   // EC stage change according to the inverse of the temperature sum between the appearance of two internodes
...
procedure TDevelopment_dl.Integrate;
...
procedure TDevelopment_dl.AddDataValueToDataSeries;
...
procedure TDevelopment_dl.AddSimValueToDataSeries;
...
procedure Register;
begin
  RegisterComponents('CERES Wheat', [TDevelopment_dl]);
end;
