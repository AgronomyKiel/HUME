...
type
  ...
  private
    ...
    procedure CalcAgeDependenLeafSenescence(var PLALR_a: real);
...
begin
  ...
  if fUseAgeDependentLeafSenescence then
   CalcAgeDependenLeafSenescence(senrate) else
  senrate := 0.0;
  PLALR_a.v := senrate;
...
begin
  ...
  *)
  ...
  begin // distribution is a function of LAI
    ...
    // limiting to range of measuremants at anthesis
...
procedure THumeWheatLeafArea.CalcAgeDependenLeafSenescence(var PLALR_a: real);
begin
  // if crop is emerged and not shooting then 5th oldes leaf is decaying wihtin one phyllochron
  if (round(ISTAGE.v) >= 1) and (round(ISTAGE.v) <= 2) and (CUMPH.v > 4) then
  // senecence only until EC 30, changed ..
  begin
    // senescence rate is only calculated once, and then remains constant
    if senratesLA[trunc(LN_.v) - 4] = 0 then
    begin
      // on the level of single leaf area, leaf area of the 5th oldest leaf is immediately decasing
    ...
    begin
      // if there is anay leaf area left on leaves older than the 5th
  ...
  begin
    ...
    begin
      PLALR_a := PSENLeaf1.v * TSumInc.v * GPLA.v // PLALR_a := 0.0;
    end
    else // PSENLeaf1.v * TSumInc.v * GPLA.v;
