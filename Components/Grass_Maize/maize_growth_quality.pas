...
Tmaize_growth_quality = class(TAbstractPlant)
...
function Tmaize_growth_quality.GetCropHeight:THumeNumEntity;

begin
  result := CropHeight;
end;

{procedure Tmaize_growth_quality.SetCropHeight(NewCropHeight:THumeNumEntity);

begin
  p_CropHeight := NewCropHeight;
end;}

function Tmaize_growth_quality.GetLAI:THumeNumEntity;
...
{procedure Tmaize_growth_quality.SetLai(NewLAI:THumeNumEntity);

begin
  p_LAI := NewLAI;
end;}

function Tmaize_growth_quality.GetNUptakeRate:THumeNumEntity;
...
{procedure Tmaize_growth_quality.SetNUptakeRate(NewNUptakeRate:THumeNumEntity);
...
function Tmaize_growth_quality.WLD_z_t_f ( z1, z2, t, Zrmax, Zr0, Kz,
                     SRLmax, SRL0, kL, ka : real): real;
...
function Tmaize_growth_quality.GetWLD(Index: Integer):THumeNumEntity;
...
{procedure Tmaize_growth_quality.SetWLD(Index: Integer; NewWLD:THumeNumEntity);
...
procedure Tmaize_growth_quality.createAll;
...
procedure Tmaize_growth_quality.init(var GlobMod: TMod);
...
procedure Tmaize_growth_quality.CalcRates;
...
//      LAI
   ...
   If  (TSumLAI.v>Tsum1.v+Tsum2.v+Tsum3.v+Tsum4.v) then
     ...
     dLAIe.v :=   dW.v*100*0.2*0.0018                 //geändert 6.8.08 100 statt 10
...
If  (quality.v>=cut1.v) or ((month=10) and (date=5)) then begin      //Ernte bei TS 32 %, spätestens 5.10.
 oldharvestdate.v:=harvestdate.v{+365.455+365.455};
 DoHarvest := true;
...
procedure Register;
begin
  RegisterComponents('MaizeModel', [Tmaize_growth_quality]);
end;
