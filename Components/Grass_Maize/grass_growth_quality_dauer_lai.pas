...
unit grass_growth_quality_dauer_lai;
...
// LAI
   ...
   LAI.v :=  W.v*0.02;       //lineare Regression DW KD 2007
...
// if ((quality.v>=cut1.v) and (W0.v=w01.v)) or ((W.v>=cutW.v)and (W0.v<>w01.v))  then begin
 if ((date=15) and (month=5)) or ((date=6)and (month=7))or ((date=16)and (month=8)) or ((date=1)and (month=10))   then begin
 ...
 decodedate(globmod.time.v, year, month, date);
if (month >=1) and (month < 5) then begin
  ...
  if (month >=5) and (month < 8)  then begin
      w0.v:=w02.v;
    rs.v:=rs2.v;
    lai50.v:=lai50_2.v;
    ak.v:=ak2.v;
