unit maize_growth_quality_N;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, UMod, UState,math,IniFiles, UAbstractPlant,
  UlayeredSoil;

Type

Tmaize_growth_quality_N = class(TAbstractPlant)
...
published
  ...
  Property Var_NDemand : TVar read NDemand write NDemand;
   Property Var_wdem : TVar read wdem write wdem;
   Property Var_snf : TVar read snf write snf;
   Property Var_di : TVar read di write di;
   Property Var_demand : TVar read demand write demand;
   Property Var_wpn : TVar read wpn write wpn;
   Property Var_npmax : TVar read npmax write npmax;
   Property Var_uptkn : TVar read uptkn write uptkn;
   Property Var_ni : TVar read ni write ni;
   Property Var_ncrit : TVar read ncrit write ncrit;
   Property Var_nsupply: TVar read nsupply write nsupply;
...
Property Par_al : TPAr read al write al;
  ...
  Property Par_ncrit_factor : TPAr read ncrit_factor write ncrit_factor;
  Property Par_ncrit_exp : TPAr read ncrit_exp write ncrit_exp;
  Property Par_mcn : TPAr read mcn write mcn;
Property Par_uplin: TPAr read uplin write uplin;
Property Par_untoti : TPAr read untoti write untoti;
Property Par_dlim : TPAr read dlim write dlim;
Property Par_npmax_e : TPAr read npmax_e write npmax_e;
Property Par_npmax_b : TPAr read npmax_b write npmax_b;
Property Par_npmax_k : TPAr read npmax_k write npmax_k;
  ...
  Property Ex_untot : TExternV read untot write untot;
...
function Tmaize_growth_quality_N.GetCropHeight:THumeNumEntity;

begin
  result := CropHeight;
end;

procedure Tmaize_growth_quality_N.SetCropHeight(NewCropHeight:THumeNumEntity);

begin
  p_CropHeight := NewCropHeight;
end;

function Tmaize_growth_quality_N.GetLAI:THumeNumEntity;
...
procedure Tmaize_growth_quality_N.SetLai(NewLAI:THumeNumEntity);

begin
  p_LAI := NewLAI;
end;

function Tmaize_growth_quality_N.GetNUptakeRate:THumeNumEntity;
...
procedure Tmaize_growth_quality_N.SetNUptakeRate(NewNUptakeRate:THumeNumEntity);
...
//  ShootN.c := p_NuptakeRate.v;
  ...
  uptkn.name := p_NUptakeRate.Name;
  uptkn.v := p_NuptakeRate.v;
...
function Tmaize_growth_quality_N.WLD_z_t_f ( z1, z2, t, Zrmax, Zr0, Kz,
                     SRLmax, SRL0, kL, ka : real): real;
...
function Tmaize_growth_quality_N.GetWLD(Index: Integer):THumeNumEntity;
...
procedure Tmaize_growth_quality_N.SetWLD(Index: Integer; NewWLD:THumeNumEntity);
...
procedure Tmaize_growth_quality_N.createAll;
...
begin
  ...
  VarCreate('GlobStr', '',0, true, GlobStr);
  ...
  VarCreate('NDemand', '[g/m2]',0, true, NDemand);
VarCreate('wdem', '[g/m2]',0, true, wdem);
VarCreate('snf', '',0, true, snf);
VarCreate('di', '',0, true, di);
VarCreate('demand', '',0, true, demand);
VarCreate('wpn', '[g/m2]',0, true, wpn);
VarCreate('npmax', '[%]',0, true, npmax);
VarCreate('uptkn', '[g/m2]',0, true, uptkn);
VarCreate('ni', '',0, true, ni);
VarCreate('ncrit', '[%]',0, true, ncrit);
VarCreate(' nsupply', '[g/m2]',0, true,  nsupply);
  ...
  ParCreate('ncrit_factor', '[-]',3.4, ncrit_factor);
  ParCreate('ncrit_exp', '[-]',-0.37, ncrit_exp);
  ParCreate('mcn', '[-]',15, mcn);
ParCreate('uplin', '[-]',1, uplin);
ParCreate('untoti', '[-]',80, untoti);
ParCreate('dlim', '[-]',0.2, dlim);
ParCreate('npmax_e', '[-]',32, npmax_e);
ParCreate('npmax_b', '[-]',6.5, npmax_b);
ParCreate('npmax_k', '[-]',0.1, npmax_k);
  ...
  ExternVCreate('untot', '',statefield, untot);
...
procedure Tmaize_growth_quality_N.init(var GlobMod: TMod);
...
procedure Tmaize_growth_quality_N.CalcRates;
...
//If (month >= 5) and (date >10) and (GlobTime.v<HarvestDate.v) then begin
...
//      LAI
   ...
   If  (TSumLAI.v>Tsum1.v+Tsum2.v+Tsum3.v+Tsum4.v) then
     ...
     dLAIe.v :=   dW.v*100*0.2*0.0018         //geändert von 10 auf 100 6.8.08
...
//water
  ...
  // WI.v :=  min(1,TransRatio.v)  ;
   WI.v :=  min(1,TransRatio.v/swthr.v)  ;  //geändert 15.5.08

 //n-model

   //npmax.v:=(npmax_e.v+(npmax_b.v-npmax_e.v)*exp(-npmax_k.v*W.v));
   npmax.v:=min(6.3,6.3*power((W.v/100),-0.42));    // in %
   demand.v:=max(0,npmax.v-wpn.v/W.v*100 ) ; //in %
   wdem.v:=demand.v/100.*W.v;  //  in g/m2
   //di.v:=1-exp(-dlim.v*wpn.v);
   //snf.v:=untot.v/untoti.v;
   //snf.v:=1;
   //nsupply.v := untot.v/10.*(1-uplin.v*exp(-WI.v*mcn.v));  // untot = MaxNUptake von SoilNitrogenUp  in kg/ha
   nsupply.v := untot.v/10.;//uplin.v*(1-exp(-WI.v*mcn.v));  // geändert 20.03.08, 13.5.08
   NDemand.v:= wdem.v;//*snf.v*di.v;  //in g/m2 für bodenmodell
   uptkn.v:=max(0,min(NDemand.v,nsupply.v));

   wpn.v:=wpn.v+uptkn.v;     //in g/m2


   if W.v > 100 then
   ncrit.v:=ncrit_factor.v*power(W.v/100.,ncrit_exp.v)    //     W.v in g/m2
   else
   ncrit.v:=ncrit_factor.v ;
   if ncrit.v <> 0 then
   NI.v:=min(1,wpn.v/W.v*100/ncrit.v)
   else
   NI.v:=1;

    GI.v :=   TI.v * RI.v * WI.v * NI.v  ;
...
decodedate(globmod.time.v, year, month, date);
If  (quality.v>=cut1.v) or ((month=11) and (date=5)) then begin      //Ernte bei TS 32 %, spätestens 5.10.
 oldharvestdate.v:=harvestdate.v;
 DoHarvest := true;
    harvestdate.v := Globtime.v;
//    NextCrop.SetSowingDate(HarvestDate.v +14);
// If self.SetNewDates = false then begin
//     sowingdate.v := 1e6; // arbitrary high number to assure that sowingdate will be set by previous crop to maize
//     harvestdate.v := 1e6;
//  end;
//Ernterückstände in N_res und C_res einfügen? //hm
//N_Residues.v:= max(5,wpn.v/10);
//C_Residues.v:= max(30,W.v/10);
 //Ernterückstände in N_res und C_res einfügen
...
procedure Register;
begin
  RegisterComponents('MaizeModel', [Tmaize_growth_quality_N]);
end;
