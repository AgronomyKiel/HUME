...
type
  ...
  private
    ...
    NminStart: real;       /// nitrate amount in soil profile at simulation start [Kg N/ha]
    NFlowInt: TSoilArray;  /// Nitratflüsse during internal time steps [Kg N/ha*d] ///
  ...
  public
    NitrateAmount: TSoilStateArray; /// Nitrat menge im Kompartiment [Kg N/ha]
    NConc: TSoilVarArray;    /// Nitrate Concentration [Kg NO3-N/cm H2O]
    Nsink: TSoilVarArray;    /// Sink term [Kg N/ha*d }
    NetMin: TSoilVarArray;   /// Source term [Kg N/ha*d }
    NFlow: TSoilVarArray;    /// Nitrate flows [Kg N/ha*d] }
    CumSumUptake : TState;   /// cumulative sum of soil N uptake
    CumSumMinera : TState;   /// cumulative sum of soil N mineralisation
    CumSumNitrateLeaching: TState; /// Sum of nitrate leaching
    Nmin0_10 : TVar;
    Nmin0_30: TVar;
    Nmin30_60: TVar;
    Nmin60_90: TVar;
    ...
    Nmin0_90: TVar;
    Nmin0_60: TVar;
    LongTermNBalance: TVar; /// long term nitrogen computational balance
    DailyNBalance : TState;   /// daily nitrogen computational balance
    TotalNBalance : TState;   /// cumulative N balance
    SumNmin: Tvar;           /// Sum of Nmin-nitrate
    NitrateLeachingRate: TVar; /// Leachingrate as daily average
    Dispersion_length: Tpar; /// Dispersion length
    Imp_factor: TPar; /// relative "implicitness" of calculation procedure
    LeachingStartDay, LeachingEndDay : TPar;  // start and end of cumulation period for leaching
    SelectedLeachingPeriod : TOption; /// Option for restricting Leaching accumulation to a certain period
...
//property ShowWarnings  : boolean read Warnings write warnings;
    ...
    property Var_SumDrain: TState read CumSumNitrateLeaching write CumSumNitrateLeaching;
    property Var_DrainageNFlow: TVar read NitrateLeachingRate write NitrateLeachingRate;
...
begin
  ...
  Nmin90_120.v := NitrateAmount[10].v + NitrateAmount[11].v + NitrateAmount[12].v;
...
begin
  ...
  StateCreate('SumNitrateLeaching', '[kg N/ha]', 0.0, false, CumSumNitrateLeaching,
    'Sum of leached Nitrate');
  ...
  StateCreate('Daily_NBal', '[kg N/ha/d]', 0.0, false, DailyNBalance, 'daily computational N balance');
  StateCreate('Total_NBal', '[kg N/ha/d]', 0.0, false, TotalNBalance, 'Total computational N balance');

  VarCreate('DrainageNFlow', '[kg N/ha/d]', 0.0, false, NitrateLeachingRate,
    'Daily nitrate-leaching rate');
  ParCreate('Dispersion length', '[cm]', 1.0, Dispersion_length, 'Parameter for hydraulic dispersion');
  ParCreate('Imp_factor', '[-]', 0.5, Imp_factor, 'Factor for implicitiness of calculation,'+
                            ' 0.5 to 1 recommended, 1 has higher numerical stability');
  ParCreate('LeachingStartDay', '[doy]', 0, LeachingStartDay, 'day when leaching sum calculation starts'+
  'works only if the option SelectedLeachingPeriod is set to true');
  ParCreate('LeachingEndDay', '[doy]', 1e9, LeachingEndDay, 'day when leaching sum calculation ends');

  OptCreate('SelectedLeachingPeriod',  'false',  SelectedLeachingPeriod, 'Should Leaching sum be calculated only '+
  ...
  VarCreate('LT_NBal', '[kg N/ha]', 0.0, false, LongTermNBalance, 'computational N balance of simulation');
  ExternVCreate('Groundwaterconc', '[kg NO3-N/cm H20]', StateField,
    Groundwaterconc, 'optional external variable with groundwater nitrate concentration');

  Groundwaterconc.Search := (LowerBoundaryCondition = Groundwatertable);

  for i := 1 to n_comp + 1 do begin
    Statecreate('Nmin' + ndx_str(i), '[kg.ha-1]', 5.0, true, NitrateAmount[i], 'Soil nitrate in layer '+inttostr(i));
    VarCreate('NKonz' + ndx_str(i), '[kg NO3/cm H20]', 0.0, false, NConc[i],'Nitrate concentration in layer '+inttostr(i));
   // NConc[i].writetofile := false;
    NConc[i].writetofile := true;
    VarCreate('NFlow' + ndx_str(i), '[kg.d-1]', 0.0, false, NFlow[i],'nitrate flow between layer '+inttostr(i-1)+ ' and layer '+inttostr(i));
  end;
...
{ Berechnung der oberen Randbedingung, wobei in der hier dargestellten
  ...
  begin
    ...
    outflow := flow_f(avg_Dsalt[1], ConcOld[1], ConcOld[2], Dist[1],
      Wflow_old[2]);
    b_vektor[1] := NitrateAmountOld[1] / WAmount[1].v
      + inflow* dt.v / WAmount[1].v - (1 - Imp_factor.v) * ( outflow) * dt.v / WAmount[1].v
  ...
  begin
    for i := 2 to n_comp - 1 do begin
      inflow := flow_f(avg_Dsalt[i - 1],ConcOld[i - 1], ConcOld[i], Dist[i -
        1], Wflow_old[i]);
      ...
      b_vektor[i] := NitrateAmountOld[i] / WAmount[i].v
...
//      mass_flow_alt := Wflow_old[i] * sqrt(ConcOld[i] * ConcOld[i - 1]);
      ...
      diff_flow_new := avg_Ds[i - 1] * (NConc[i - 1].v - NConc[i].v) /
        Dist[i];
//      mass_flow_new := WflowInt_arr[i].v * sqrt(NConc[i - 1].v * NConc[i].v);
    ...
    diff_flow_alt := avg_Dsalt[n_comp]*(ConcOld[n_comp] - ConcOld[n_comp+1]) /
        Dist[n_comp];
    mass_flow_alt := Wflow_old[n_comp+1] * (ConcOld[n_comp+1] + ConcOld[n_comp])/2;
  ...
  begin
    ...
    for i := 1 to n_comp do begin
      NewAmount := NConc[i].v * theta_new[i]*Thick[i];  // new nitrate amount from concentration and water amount
      if NewAmount < 0.0 then
        NewAmount := 0.0;
      net_flow[i] := (NitrateAmount[i].v - NewAmount) / dt.v + (Nsink[i].v +
        NetMin[i].v);
    ...
    if lowercase(SelectedLeachingPeriod.Option) = 'true' then begin
      if (GlobTime.v >= LeachingStartDay.v) and (GlobTime.v < LeachingEndDay.v ) then
        CumSumNitrateLeaching.c := CumSumNitrateLeaching.c + NflowInt[trunc(bil_nr.v) + 1] * dt.v else
        CumSumNitrateLeaching.c := 0.0;
    end else
      CumSumNitrateLeaching.c := CumSumNitrateLeaching.c + NflowInt[trunc(bil_nr.v) + 1] * dt.v;
...
//    DiffNmin := (SumNmin.v - NminOld);
    DailyNBalance.c := DailyNBalance.c + DiffNmin + NflowInt[trunc(bil_nr.v)+1]*dt.v - SumUptake*dt.v - SumMinera*dt.v;
...
begin
  ...
  begin
    if GlobMod.IniFileNames.IndexOf(GlobMod.ActIniFile.FileName)
      ...
      exit;
    NextINI := TMyIniFile.create(GlobMod.IniFileNames[GlobMod.IniFileNames.IndexOf
 ...
 //       (GlobMod.ActIniFile.FileName) + 1]);
    NextStateINI := TMyIniFile.create(NextINI.ReadString('FileNames', 'StateIniFN', ''),TEncoding.UTF8 );
