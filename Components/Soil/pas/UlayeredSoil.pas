...
type
real = double;
TSoilStateArray  = array[0..max_comp+1] of TState;    /// basic array type for soil layer state variables
TSoilVarArray    = array[0..max_comp+2] of TVar;      /// basic array type for soil layer variables
TSoilExtArray    = array[0..max_comp+1] of TExternV;  /// basic array type for soil layer external variables
TSoilArray       = array[0..max_comp+1] of real;      /// basic array type for soil layer floating point fields
TTexture_version = (RR, KA);       /// type for pedotransfer function RR= Rote Reihe
...
public
     ...
     MaxProfileDepth  : TPar;       /// Tiefe bis zu der berechnet wird [cm]}
     Potenz_f  : TPar;             /// power factor for uneven layer thickness
     dz        : real;             /// thickness of layer}
     Dist      : TSoilArray;       /// Vektor der Abstaende der Kompartimentmittelpunkte [cm] }
     Thick     : TSoilArray;       /// Vektor der Kompartimentdicken [cm] }
     Depth     : TSoilVarArray;       /// Abstand der unteren Kompartimentgrenze von der Bodenoberfläche in [cm] }
     ...
     Texture_versionOption: TOption;   ///  Version of pedotransfer function to be used
...
begin
  inherited CreateAll;
  n_comp := 20;
  ParCreate('MaxTiefe', '[cm]', 200.0, MaxProfileDepth, 'maximum depth of calculated profile');
  ParCreate('Potenz_f', '[]', 1.0, Potenz_f, 'factor for uneven layer depth distribution');

  for i := 0 to n_comp+2 do begin
    VarCreate('Tiefe'+inttostr(i),'[cm]',0.0,false, Depth[i]);     //richtig
    Depth[i].writeToFile := false;
  end;
  OptCreate('Texture_version', 'KA', Texture_versionOption, 'Option for choosing different pedotransfer functions, RR="Rote Reihe", "KA"=addendum to KA5');
