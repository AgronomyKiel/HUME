  ...
  begin
    ...
    begin
      ...
      if Overflow > 0 then
       ...
       PondedWater.v:= PondedWater.v + CumRunoff.c + Overflow * 10 * GlobTime.c;
    end;
...
var
  ...
  begin
   ...
   if (theta_neu[1] < WPar[1].b_rest) and (WflowInt_arr[1].v <= 0.0) then
    ...
    begin
      avg_Dw[0] := (Wpar[1].Dw_f((Wpar[1].b_sat + theta_arr[1].v)/2));     // aritmethic mea
      Dw_fact[1] := avg_Dw[0] * dt.v / Abst[1];

      B_vektor[1] :=
        theta_arr[1].v
          - Ku_fact[2] / Dicke[1]
            - Sink_arr[1].v * dt.v / Dicke[1];
      lower[1] := -Dw_fact[1] / Dicke[1];
      diag[1]  := Dw_fact[2] / Dicke[1] + 1.0;
      upper[1] := -Dw_fact[2] / Dicke[1];
   end;
  end;
