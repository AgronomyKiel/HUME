...
begin
   if OptSinkTermMethod = Feddes then begin
    ...
    for i := 1 to (n_comp-1) do begin
      rPAW:= ((theta_arr[i].v-WPar[i].b_rest))/nFK_arr[i];
    ...
    // Staunässe nach Feddes
     If psi_arr[i].v < 1 then  red_f :=max(0.1,psi_arr[i].v);
      rPAW:= ((theta_arr[i].v-WPar[i].b_rest))/nFK_arr[i];
	    If ((red_f < 0.0) or (rPAW < nfk_threshold.v) {or (ProzNFK0_Weff.v<1)}) then
			  red_f := 0.0;
			SinkRedf[i] := red_f;
	end;
  end; // Feddes end
  ...
  if ShowWarnings then
    ...
    for I := 1 to act_n_comp do begin
      case f_SqrWl_funct of
        NoReductionFactor:
          Sqr_wl_arr[i] := power(ExWLD_arr[i].v*Dicke[i], CompFactor.v);
        ReductionFactor:
          Sqr_wl_arr[i] := SinkRedf[i]*power(ExWLD_arr[i].v*Dicke[i], CompFactor.v);
      end;
      Sum_wl := Sum_wl+Exwld_arr[i].v*Dicke[i];
      Sum_Sqr_wl := Sum_Sqr_wl+Sqr_wl_arr[i];
    end;

    for I := 1 to act_n_comp do begin
      if sum_wl>0 then
        wl_fact[i]    := Exwld_arr[i].v*Dicke[i]/sum_wl
      else
        wl_fact[i]    := 0.0;
...
{    if psiRoot.v > 0 then
    ...
    for I := 1 to act_n_comp do begin
      ...
      if sink_arr[i].v > (((theta_arr[i].v-WPar[i].b_rest)*Dicke[i])-nfk_threshold.v) then
        sink_arr[i].v := ((theta_arr[i].v-WPar[i].b_rest)*Dicke[i])-nfk_threshold.v;
  ...
  if Autoirrioptstr.Option = 'yes' then begin
     if AutoirriMethodoptstr.Option = 'amTransRatio' then begin
       if (self.TransRatio.v < 0.99) then begin
         WMenge[1].v := WMenge[1].v + self.IrriAmount.v/10*dt.v;
...
begin
  ...
  If GlobTime.v > GlobMod.Starttime then begin
    ...
    CumAbsWaterBalance.c := abs(CumWaterBalance.c);
  end;
