unit USoilWaterMod_save; { Stand 19.4.09, Grundwasser eingebaut - Ulf Bˆttcher }
...
type
  ...
  TLowerBoundaryCondition = (NoFlow, Flow, Content, Groundwatertable); /// type for specifying lower boundary conditions
  TCompMethod = (Capacity, Potential, Richards); /// type for specifying computation method
  ...
  private
    ...
    Dw_fact, Ku_fact, B_vektor, lower, diag, upper,
  ...
  public
    ...
    HoriNdx1, HoriNdx2, HoriNdx3, HoriNdx4: TPar; /// Index der untersten Schicht im jeweiligen Horizont
    ...
    WG0_30, WG30_60, WG60_90, WG90_120, WG0_60,
      ...
      WG0_10, WG0_20,WG0_40, WG10_30, WG20_30, WG20_40, WG30_40, WG30_50, WG30_120, WG30_100,
...
begin
  ...
  ParCreate('Max_dt', '[d]', 1, max_dt, 'maximum time step length for internal calculation'); // Index f¸r Bilanzierung
  ParCreate('Max_aenderWG', '[d]', 0.01, Max_aenderWG, 'maximum time step length for internal calculation'); // Index f¸r Bilanzierung
  ...
  OptIniMethod.OptionList.Add('Parameter');
    if self.Opt_IniMethod = Watercontents then
    self.OptIniMethod.Option := 'watercontents';
  if self.Opt_IniMethod = Potentials then
    self.OptIniMethod.Option := 'potentials';
  if self.Opt_IniMethod = Parameter then
    self.OptIniMethod.Option := 'parameter';
...
// set "property Variable" according to option choise in Options.ini
  if self.OptIniMethod.Option = 'watercontents' then
    self.Opt_IniMethod := Watercontents;
  if self.OptIniMethod.Option = 'potentials' then
    self.Opt_IniMethod := Potentials;
  if self.OptIniMethod.Option = 'parameter' then
    self.Opt_IniMethod := Parameter;
  ...
  else if self.Opt_IniMethod = Potentials then
  begin
    for i := 1 to n_comp + 1 do
    begin
  ...
  else if self.Opt_IniMethod = Parameter then
  begin
    for i := 1 to n_comp + 1 do
    begin
  ...
  if uppercase(OptUntere_Randb.Option) = 'CONTENT' then
    Untere_Randb := content;
...
begin
  WflowInt_arr[1].v := -0.1 * Act_Evap.v + 0.1 * TExternV(NetRain).v;
...
begin
  ...
  until SumOfInternalTimeSteps.v >= self.GlobTime.c;
...
begin
  ...
  if (TExternV(NetRain).v > 0.0) then
  begin
...
var
  ...
  begin
    ...
    begin { weitere Iterationen }
      ...
      for i := 2 to n_comp + 1 do
      begin
        {avg_Dw[i] := min (Dw_arr[i - 1] , Dw_arr[i]);//(Dw_arr[i - 1] + Dw_arr[i]) / 2.0;    //cm2/d
    ...
    if self.FSoilHeatModel <> nil then
    begin
      for i := 1 to n_comp + 1 do
      begin
  ...
  begin
    ...
    begin
      ...
      WflowInt_arr[1].v := theta_neu[1] -
        (theta_arr[1].v - Sink_arr[1].v * dt.v / Dicke[1]);
      start := 2;
    end;
    ...
    begin
      success := true;
      start := 1;
      B_vektor[1] := theta_arr[1].v + WflowInt_arr[1].v * dt.v / Dicke[1]
        - Ku_fact[2] / Dicke[1] - Sink_arr[1].v * dt.v / Dicke[1];
  ...
  begin
    ...
    begin
      B_vektor[i] := theta_arr[i].v - Ku_fact[i + 1] / Dicke[i] + Ku_fact[i]
        / Dicke[i] - Sink_arr[i].v * dt.v / Dicke[i];
  ...
  begin
    if (Untere_Randb = content) or (Untere_Randb = Groundwatertable) then
      ...
      B_vektor[n_comp] := theta_arr[n_comp].v + Ku_fact[n_comp] / Dicke[n_comp]
        - WflowInt_arr[n_comp + 1].v / Dicke[n_comp] - Sink_arr[n_comp].v * dt.v
        / Dicke[n_comp];
  ...
  begin
    ...
    begin
     ...
     // if theta_neu[i] < 1E-20 then     begin
     //   showmessage('Fehler: WMenge_' + inttostr(i) + ' = 0');
     //   showmessage('Fehler: WMenge_' + floattostr(globtime.v) + ' = 0');
     // end;
    end;
  end;
  ...
  begin
    overflow := 0;
    for i := n_comp downto 1 do
    begin
      if theta_neu[i] > WPar[i].b_sat then
      begin
        // save overshooting amount of water
        Overflow :=  (theta_neu[i] - WPar[i].b_sat) * Dicke[i];   // Wasserdifferenz in cm
        theta_neu[i] := WPar[i].b_sat;
        WflowInt_arr[i-1].v := WflowInt_arr[i-1].v - Overflow / dt.v;
        // reduce the inflow by the amount of water which has could not stored minus the downward flux to the next layer
 //       WflowInt_arr[i].v := Wflowint_arr[i-1].v + Overflow / dt.v;
        layer := i-1; // start with the nex higher layer to the one where overflow occurred
        repeat
          if theta_neu[layer] < WPar[layer].b_sat then   // Is there some capacity left over in the next upper layer?
          begin // water capacity available?
            maxstorage := (WPar[layer].b_sat - theta_neu[layer]) * Dicke[layer];     // freie Kapazit‰t [cm], kann auch negativ sein ...
            // how much?
            ...
            begin // everything fits in this layer ?
              ...
              WflowInt_arr[layer-1].v := WflowInt_arr[layer-1].v - maxstorage / dt.v;
            end
            else   // all overflow fits into this layer
            begin
              theta_neu[layer] := theta_neu[layer] + Overflow / Dicke[layer];
              if layer > 2 then
                WflowInt_arr[layer-1].v := WflowInt_arr[layer-1].v - overflow / dt.v
              else
                if Overflow > 0 then
                CumRunoff.c := CumRunoff.c + Overflow * 10 * dt.v/globtime.c { cm -> mm } ;
 //             WflowInt_arr[layer].v := WflowInt_arr[layer].v + WflowInt_arr[layer+1].v - Overflow / dt.v;
//              WflowInt_arr[layer].v := WflowInt_arr[layer+1].v + Overflow / dt.v;
              Overflow := 0.0;
            end;
          end;
 //         WflowInt_arr[layer].v := WflowInt_arr[layer].v - Overflow / dt.v;
          dec(layer)   // next upper layer
        until (layer = 0) or (Overflow <= 0);
        // all overflow distributed or surface layer reached ...
      end;
      if theta_neu[i] < 0.0 { Wpar[i].b_rest } then
      begin
  ...
  begin
    ...
    if Untere_Randb = Flow then begin
       theta_arr[n_comp+1].v := WPar[n_comp+1].b_psi_f(psi_arr[n_comp].v- Abst[n_comp]);
...
var
  ...
  const
    ...
    { zur Verhinderung von ung¸ltigen Funktionsaufrufen wird zuerst
      ...
      in den Variablen "Wet", bzw. "Dry" gespeichert. }
  begin
    if (WPar[1].b_psi_f(psi_arr[1].v) + NetRain.v * dt.v > WPar[1].b_psi_f
        (20000)
      { AirDryness } ) { and (psi_arr[1].v> 1) } then
    begin
      { Wasserspannungen im erlaubten Rahmen ? }
      start := 1;
      { flow_arr[1] := soll_inflow; }
      B_vektor[1] := psi_arr[1].v + WflowInt_arr[1].v * P[1] - avg_Ku[1] * P[1]
        - Sink_arr[1].v * P[1];
  ...
  const
    MinPsi = 1;
  begin
    for i := n_comp downto 1 { Start } do
    begin
      if theta_neu[i] > WPar[i].b_psi_f(MinPsi) then
      begin
  ...
  var
    ...
    d_WaGe, { énderung des Wassergehaltes
      im Kompartiment                  [cm3/cm3] }
    sum_d_WaMe, { Summe der Wassermengen‰nderungen   [cm] }
    sum_sink { Summe der Sink-Terme            [cm] }
    : real;
  ...
  begin
    ...
    begin
      ...
      { Der folgende Algorithmus wurde eingef¸gt, um Diskontinuit‰ten bei der
        Verwendung von Eingabedaten auf t‰glicher Basis zu vermeiden. }
      if SumOfInternalTimeSteps.v + dt_neu > GlobTime.c then
