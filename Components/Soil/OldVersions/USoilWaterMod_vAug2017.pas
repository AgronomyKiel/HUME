unit USoilWaterMod_vAug2017; { Stand 19.4.09, Grundwasser eingebaut - Ulf Böttcher }
...
const
  max_iter_error = 0.0001;  /// maximum error between two iterations
...
type
  ...
  private
    ...
    avg_Ku, /// Mittelwert der unges„ttigten hydr. Leitfähigkeit zwischen 2 Kompartimenten [cm/d] }
...
begin
  ...
  ParCreate('Max_dt', '[d]', 1, max_dt, 'maximum time step length for internal calculation'); // Index für Bilanzierung
  ParCreate('Max_aenderWG', '[d]', 0.01, Max_aenderWG, 'maximum time step length for internal calculation'); // Index für Bilanzierung
...
begin
  if (iter = 0) and (CompMethod = Potential) then begin { lineare Extrapolation der Wassergehalte bei der ersten Iteration }
...
//      psi_arr[i].v := WPar[i].psi_b_f(est_theta[i]);
    ...
    for i := 2 to n_comp + 1 do
       begin
         {avg_Dw[i] := min (Dw_arr[i - 1] , Dw_arr[i]);//(Dw_arr[i - 1] +
           Dw_arr[i]) / 2.0;    //cm2/d
...
var
 ...
 // ToDo ist hier die Berechnung korrekt? Netrain noch einbeziehen??
  ...
  begin
    ...
    begin
      ...
      begin
        Overflow := overflow + (theta_neu[i] - WPar[i].b_sat) * Dicke[i];
        // save overshooting amount of water
        theta_neu[i] := WPar[i].b_sat;

        if (WflowInt_arr[i+1].v < 0) and (abs(WflowInt_arr[i+1].v) > abs(WflowInt_arr[i].v)) then
          isupflow := true;
        if self.WflowInt_arr[i].v>0 then
          isdownflow := true;
        if isupflow then begin
        layer := i+1;
        repeat
           WflowInt_arr[layer].v :=  WflowInt_arr[layer].v+Overflow/dt.v;
           maxstorage :=  (WPar[layer].b_sat - theta_neu[layer] ) * Dicke[layer];
           if maxstorage>overflow then begin
           ...
           inc(layer);
        until ((layer = n_comp) or (Overflow <= 0));
        end
        else begin
        layer := i-1; // start with the lowest layer where overflow occurred
        repeat
       ...
       //   WflowInt_arr[layer].v := WflowInt_arr[layer].v - Overflow / dt.v;
        ...
        if (Overflow > 0) and (layer =0) then  begin
          CumRunoff.c := CumRunoff.c + Overflow * 10 / dt.v;
          Overflow := 0.0;
        end;
      end;
 ...
 // bisherige Berechnung aus CalcSinks
 // WflowInt_arr[1].v := -0.1 * Act_Evap.v + 0.1 * TExternV(NetRain).v;
//    wflowInt_arr[1].v := wflowint_arr[1].v+overflow/dt.v; ;
  end;
...
var
  ...
  begin
    ...
    begin { sonst keine weiteren Änderungen der Wasserspannung }
