...
type
  ...
  TSinkTermMethod = (nFK_crit, Psicrit, Psicrit_corr, Feddes);
...
begin
  ...
  if OptSinkTermMethod = Psicrit_corr then begin
    for i := 1 to (n_comp-1) do begin
      ...
      if (fPsi2Opt = fromPlantmodel) and IsPlantModelSet
       ...
       If psi_root[i] < Psi2.v then
        red_f := 1.0 else
        red_f := (log10(psi_root[i])-log10(psi_3.v))/(log10(Psi2.v)-log10(psi_3.v));
      If ((red_f < 0.0) or (rPAW < nfk_threshold.v)) then
        red_f := 0.0;
      SinkRedf[i] := red_f;
     end else
