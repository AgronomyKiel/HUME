...
uses
  Ustate, UlayeredSoil, UMod, classes, UAbstractSoilMin, IniFilesNew;
...
type
  ...
  processes = (dpm_biom, rpm_biom, som_biom, biom_som);  /// transfer processes between the pools:

  TPoolStateArray = array [Pools] of TState;  /// Type for state var sums over layers
  TPoolVarArray = array [Pools] of TVar;   /// Type for var sums over layers
  TPoolParArray = array [Pools] of TPar;  /// Type for var sums over layers
  ...
  public
    ...
    product   /// product pool of the process
    : Pools;
  ...
  public
    ...
    WFPS_Arr: array [1 .. MaxNOrgLayers] of TVar;  /// array with values of water filled pore space
    ...
    CFlowArr: array [processes, 0 .. MaxNOrgLayers] of TVar; /// matrix with C flows
    CO2FlowArr: array [processes, 0 .. MaxNOrgLayers] of TVar; /// matrix with CO2 flows
    ...
    CBilanz /// balance of carbon
      : TVar;
    NetMinArr: array [processes, 0 .. MaxNOrgLayers] of TVar;  /// matrix with net mineralisation rates
...
begin
  ...
  Nr := C_flow_educt * (1 / CN[Edukt].v - E.v / CN[product].v);
  if Nr <= 0.0 then
  begin
  ...
  begin
    CPools[Edukt].c := 0.0;
    CPools[product].c := 0.0;
    Nr := 0.0;
  end;
  CPools[Edukt].c := CPools[Edukt].c - C_flow_educt;
  CPools[product].c := CPools[product].c + C_flow_product;
...
begin
  ...
  begin
    ...
    // if immobilisation is larger than available Nmin then mineralisation rate has to be reduced !
