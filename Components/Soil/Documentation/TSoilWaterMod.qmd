...
The sind term S\[i\], describes the water uptake by the plant roots in a certain layer \[i\], depends on Tpot, a reduction factor \[-\] (f~sinkred~) and a term, which describes the reduction of water uptake by roots due to competition. This term is calculated by the root length in the layer \[cm cm^-2^\] (RL\[i\]), an exponential factor for root competition \[-\] (CompF) divided by the sum of root length in all layers \[cm cm^-2^\] (RL\[j\]) and CompF. 0.1 is simply the conversion from cm to mm.

$$S[i]=0.1\cdot {{T}_{pot}}\cdot \frac{RL{{[i]}^{CompF}}}{\sum\limits_{j}{RL{{[j]}^{CompF}}}}\cdot {{f}_{\sin kred}}$$ {#eq-SinkTerm}
