...
The component TSoilWaterModelR implements some relatively simple approaches for describing these complex processes but efforts have been started to include a more mechanistic approach for simulation of root water uptake. The development started with the implementation of a simple reduction factor for root water uptake based on the soil water potential in the different soil layers and the potential transpiration rate. This approach is similar to [@feddes1978] but with a simpler calculation of the reduction factor for root water uptake.

## Implementation

The root water uptake is considered in the water balance equation as a sink term (S). The potential transpiration (T~pot~) is by default calculated based on the Penman-Monteith equation in the component [TPenMonteith](../../Evapotranspiration/Documentation/TPenMonteith.qmd), which considers the atmospheric demand for water and the resistance to water flow from the soil to the atmosphere through the plant.
...
## Options inherited from `TSoilWaterMod` {#sec-soil-water-base-options}

`TSoilWaterModelR` inherits the general soil-water configuration registered by `TSoilWaterMod.CreateOptions` in `USoilWaterMod.pas`. The option names below are the exact keys used in the options INI file, including spaces and question marks. Defaults and accepted values are taken from the current implementation.
...
The `SinkTermMethod` option selects the calculation of actual transpiration (T~act~) and the sink term for the different soil layers:
...
- psicrit~corr~: The reduction factor for root water uptake (f~sinkred~) is calculated from an estimate of the soil water potential at the root surface. This estimate depends on the water influx rate and the unsaturated hydraulic conductivity near the roots and is based on the steady-state radial-flow solution of @Gardner1960.

- Feddes: The reduction factor for root water uptake (f~sinkred~) is calculated based on the soil water potential in each soil layer (psi~i~) and the actual transpiration rate (T~act~), following a Feddes-like approach [@feddes1978].

- MFP: Water uptake is limited by the maximum flow towards roots calculated from the matrix flux potential [@vanLier2013; @Metselaar2011]. Two additional options independently select the spatial resolution and the distribution of potential uptake.

For `SinkTermMethod = MFP`, the following options apply:

- `WithClasses`: If `false` (default), one mean root length density and soil water content are used per soil layer. If `true`, each layer is divided into ten root length density classes supplied by [TSimpleRootModDM](../../Rootgrowth/Documentation/TSimpleRootModDM.qmd), and uptake is calculated separately for every class.

- `MFPuptakedistribution`: With `empirical` (default), potential uptake is distributed using the root-length and competition-factor weighting also used by the empirical sink methods. With `MaximumFlow`, potential uptake is distributed according to each layer's or class's relative maximum MFP uptake rate.

The former `MFPvar` sink method is replaced by `SinkTermMethod = MFP`, `WithClasses = true`, and `MFPuptakedistribution = MaximumFlow`.
...
The default option is to distribute the potential transpiration according to the fraction of root length in a soil layer. The root length in the different soil layers is calculated by the component [TSimpleRootModDM](../../Rootgrowth/Documentation/TSimpleRootModDM.qmd) and is provided as an external value to the component TSoilWaterModelR.
...
$$\psi_{2,low} = max\left(\psi_2+\frac{\psi_{2,diff}}{2}, \psi_{PWP}\right)$$
...
The theoretical background of the more mechanistic approaches is based on the calculation of water transport towards roots, for which the single root approximation (@fig-SingleRootCylinder1) is the most used geometrical simplification, reducing the complex system of a real branched root system taking up water from a layered soil into the calculation of the one dimension transport in radial coordiates (@Gardner1960, @Philip1957, @Carslaw1946, @vanLier2006, @Metselaar2011 and @vanLier2013).

The single root or 'equivalent' cylinder assumes the roots in one layer are organized in equally spaced parallel array. There are no analytical solutions of the diffusion equation sufficiently simple to be of general use for this case (see @Youngs1963), and numerical solutions or approximations have to be employed. The latter are most useful for discussing the general features of the system. @Passioura1968 have discussed the two usual methods, the 'steady state' and 'steady rate' approximations.
...
@Gardner1960 and @Philip1957 first gave analytic solutions for boundary conditions given by (a) a constant rate (I~w~) of water uptake per unit root length $[cm^3cm^{-1}d^{-1}]$ where the inner boundary condition is given by:
...
at r = a, and with the assumption of an infinite soil volume available for the sink, neglecting root competition, the solution is (@Philip1957, @Carslaw1946):

$$\overline{\psi_s}-\psi_a=\frac{I_w}{4\pi K_u}\left[ln\left(\frac{4D_wt}{a^2}\right)-0.5772\right]$$ {#eq-CarslawJaeger}

where $\overline{\psi_s}$ and $\psi_a$ are the water potentials at the root surface and in the bulk soil, and a is the root radius. The value 0.5772 is the so called Euler-Mascheroni constant.

A similar formulation based on diffusivity and for water content differences is given by @vanLier2006: 

$$\overline{\theta_s}-\theta_a=\frac{x^2T_p}{4z D_w}\left[ln\left(\frac{4D_wt}{a^2}\right)-0.5772\right]$$ {#eq-QuirindeJong2006}

The first terms on the right hand side of Eqs. @eq-CarslawJaeger and @eq-QuirindeJong2006 contain different formulations of the dimensionless change of water content per day, which are, however, equivalent and can be transformed into terms which have the units of $d^{-1}$.
...
$$\frac{\Delta \theta}{\Delta t}=\frac{I_w}{\pi (x^2-a^2)}\approx \\
\frac{I_w}{\pi x^2} $$ {#eq-Thetadt}

and recalculating with the new values of $\psi_b$ $K_u$ or $D_w$ to allow for the change in $\theta$.

The steady rate approximation (@Cowan1965) is rather more realistic, since it embodies the assumption that water is being supplied uniformly from within the soil cylinder to maintain the constant flow (@fig-SingleRootCylinder1). The equation for this system is
...
The solutions are therefore closely similar, but, for constant $\theta_b$, the steady rate equation predicts a value of $\theta_a$ (for constant $\theta_b$) which is identical to that of the steady-state equation at r = 1.65a. The equations are most useful in the relation to the mean water content $\overline\theta$, rather than $\theta_b$ and then give (@Passioura1968) a steady-state approximation for x \>\> a, which is valid for most cases:
...
Rearranging for I~w~ gives:

$$I_w=2\pi k_u \cdot \frac{\psi_a-\overline{\psi}}{ln\frac{x}{1.65a}}$$ {#eq-SteadyStatepsi2}

### Steady rate solution

The steady rate approximation (@Cowan1965) is rather more realistic, since it embodies the assumption that water is being supplied uniformly from within the soil cylinder to maintain the constant flow. The steady-rate solution and its approximation for x \>\> a is (@Tinker1976):
...
In @fig-WcontProfilesQ the water content distribution around roots is shown for different values of the water uptake rate per unit root length (I~w~) and for different values of the soil water diffusivity (D~w~), as affected by texture and volumetric soil water content, which decreases during the simulated extraction of water from an isolated single root cylnder. The water content at the root surface is lower than in the bulk soil, and the difference increases with increasing water uptake rate per unit root length (I~w~) and decreasing soil water diffusivity (D~w~). The silty soil (Ut2) with the higher soil water diffusivity (D~w~) shows a smaller difference between the water content at the root surface and in the bulk soil than the sandy soils (Sl3 and especially gS) with the lower soil water diffusivity (D~w~).
...
The matric flux potential (M) is a mathematical integration of unsaturated hydraulic conductivity over soil matric potential. It simplifies the non-linear governing equations of unsaturated soil water transport (such as Richards' equation) by combining conductivity and tension into a single scalar potential variable.

@vanLier2006 with correction by @Schroder2007 derived an expression to describe M as a function of the radial distance from the root surface r \[cm\], based on the continuity equation for one‐dimensional axisymmetric flow Matric flux potential $(M) \left[ {cm^{2} d^{−1}} \right]$ is defined as the integral of unsaturated hydraulic conductivity, $K(h) [cm^.d^{−1}]$, over pressure head, or equivalently, as the integral of diffusivity, $D(θ) \left[cm^2d^{−1}\right]$, over volumetric water content $(\theta) \left[cm^{3}cm^{−3}\right]$. For convenience, the permanent wilting point in terms of pressure head $(h_w) \left[cm\right]$ or water content $\theta{_w} \left[cm^{3}cm^{−3}\right]$ is chosen as the lower bound of the integral:

$$M=\int_{-h_w}^{h}K(h)dh=\int_{\theta{_w}}^{\theta}D(\theta)d(\theta)$$ {#eq-MFPdef}

The Matric flux potential can be calculated numerically by integrating $k_u$ or $D_w$ from the permanent wilting point. @fig-MFluxPotCurves gives the curves for the matrix flux potential for three chosen soil textures (gS: coarse sand, Sl3: loamy sand, Ut2: clayey silt).
...
Solutions of @eq-DiffusionSingleRoot using the matric flux potential approach were derived by @de_Willigen1987 and were more recently used to derive sink term formulation of macroscopic soil water transport models by @vanLier2006, @Schroder2007 and @vanLier2008.
...
where $\overline{\theta}$ is the average water content in the entire rhizosphere and z \[cm\] is the rooting depth.Assuming that $\frac{d\theta}{dt}$ is independent of r Eq. can be generalized:
...
$$-\frac{T_p}{z}=\frac{I_wL}{z}=-\frac{-q}{r}-\frac{\partial q}{\partial r}=\frac{\partial M}{r\partial r}+\frac{{{\partial }^{2}}M}{\partial {{r}^{2}}} $$ {#eq-Tpz3} 

For this equation the following solution is found (@vanLier2006 eq. 39):

$$M=\frac{-T_p}{4z}r^2+C_1ln(r)+C_2 $$ {#eq-MbaseIntegral}

where $C_1$ and $C_2$ are integration constants. This solution is known as the steady-rate solution. We assume the continuum approach required to apply Darcy's equation to hold for the geometry and the scales considered. @vanLier2006 and @Schroder2007 used the boundary condition
...
$$\frac{dM}{dr}=T_p\frac{x^2}{2za};\;\;\;\;\;r=a $$ {#eq-dMdr} where x (cm) is the rhizosphere radius, which is equal to the half mean distance between roots and is related to the root length density $L_{rv}\;[cm\;cm^{−3}]$ by $x=(\pi L_{rv})^{-1/2}$.
...
@Metselaar2007 showed by numerical analysis that M(r) under limiting hydraulic conditions has the same shape as under non-limiting conditions and may be described with an expression, with $T_p$ replaced by the actual transpiration rate $T_a$ and $M_0$ equal to the matric flux potential at the permanent wilting point ($M_w$), which by definition is equal to zero under limiting soil water conditions. Therefore, in the falling rate phase,
...
Using analytic methods, the value of d was shown to be $e^{−1/2}$ (≈0.607) assuming a constant diffusivity by @vanLier2006 For soils with van Genuchten hydraulic functions, these authors showed with numerical simulations that the value of d is slightly lower (0.56 ± 0.06, with a median value of 0.53). Substituting $\overline{M}$ and $\overline{r}$ into Eq. \[18\] and incorporating Eq. \[19\] yields
...
```{r}
#|label: fig-WcontProfilesMFP
#|echo: FALSE
#|message: FALSE
#|warning: FALSE
#|fig.align: 'center'
#|fig.cap: "Water content distribution around roots at different soil water potentials and for three different soil textures calculated with the matrix flux potential model. Upper dotted line: field apacity, lower dotted line: permanent wilting point"
...
### Comparison of the steady rate and matric flux potential approach

In @fig-SingleRootSimulation1 the water content distribution around a single root cylinder is shown for the steady rate and matric flux potential approach during a simulated water extraction period as an animated gif. As during the simulated water extraction period the soil water content decreases, the difference between the water content at the root surface and in the bulk soil increases. The difference is larger for the steady rate approach than for the matric flux potential approach, which is more realistic as it accounts for the decrease of soil water diffusivity with decreasing soil water content.

![Simulation of water uptake from a single root cylinder](images/swc_rad.gif){#fig-SingleRootSimulation1}

### Implementation of the matric flux potential approach in the TSoilWaterModelR

The root length density used by the `MFP` sink method is the hydraulically active fraction of the available root length density:

$$L_{rv,act}=f_{act}L_{rv}$$ {#eq-active-rld}

Here, $f_{act}$ is the parameter `FracActRoots` \[-\], with a default value of 0.3. The active root length density determines both the mean distance between active roots and the maximum uptake rate. With classes enabled, the factor is applied separately to every root length density class. Other water sink methods are unaffected, and the factor is not yet applied to nitrate uptake.

Potential transpiration is first allocated to the selected hydraulic units (soil layers or root length density classes). Actual uptake is then limited independently in every unit:

$$S_{act,k}=min\left(S_{pot,k},S_{max,k}\right)$$ {#eq-MFP-actual-sink}

The two MFP options give four symmetric combinations:

| `WithClasses` | `MFPuptakedistribution` | Potential-uptake allocation | Hydraulic limitation |
|---|---|---|---|
| `false` | `empirical` | Empirical root-length and competition-factor weights between layers | Layer-average MFP capacity |
| `false` | `MaximumFlow` | Relative maximum MFP capacity of each layer | Layer-average MFP capacity |
| `true` | `empirical` | Empirical root-length and competition-factor weights between all classes | Class-specific MFP capacity |
| `true` | `MaximumFlow` | Relative maximum MFP capacity of each class | Class-specific MFP capacity |

## Approach for non-uniformly distributed roots
