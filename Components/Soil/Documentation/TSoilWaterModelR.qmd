...
The first terms on the right hand side of the equations @eq-CarslawJaeger and @eq-QuirindeJong2006) contain different formulations of the dimensionless change of water content per day, which are, however, equivalent and can be transformed into terms which have the units of $d^{-1}$.
...
In the implementation, the root length density used by the `MFP` and `MFPvar` sink methods is the hydraulically active fraction of the available root length density:

$$L_{rv,act}=f_{act}L_{rv}$$ {#eq-active-rld}

Here, $f_{act}$ is the parameter `FracActRoots` \[-\], with a default value of 0.3. The active root length density is used both to derive the mean distance between active roots and to scale their maximum water uptake. For `MFPvar`, the factor is applied separately to every root length density moment. Other water sink methods are unaffected, and the factor is not yet applied to nitrate uptake.
