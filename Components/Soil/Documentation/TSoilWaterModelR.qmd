...
- `WithClasses`: If `false` (default), one mean root length density and soil water content are used per soil layer. If `true`, each layer is divided into ten root length density classes supplied by [TSimpleRootModDM](https://github.com/AgronomyKiel/HUME/blob/mainbranch/Components/Rootgrowth/Documentation/TSimpleRootModDM.html), and uptake is calculated separately for every class.
...
The steady rate approximation (@Cowan1965) is rather more realistic, since it embodies the assumption that water is being supplied uniformly from within the soil cylinder to maintain the constant flow. The Steady rate solution and its approximation for for x \>\> a is (@Tinker1976):
...
The Matric flux potential can be calculated numerically by integrating $k_u$ or $D_w$ from the permanent wilting point. @fig-MFluxPotCurves gives the curves for the matrix flux potential for three choosen soil textures (gS: coarse sand, Sl3: loamy sand, Ut2: clayey silt).
...
A solutions of @eq-DiffusionSingleRoot using the matrix flux potential approach have been derived by @de_Willigen1987 and were more recently used to derive sink term formulation of macroscopic soil water transport models by @vanLier2006, @Schroder2007 and @vanLier2008.
