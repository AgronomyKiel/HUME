...
There are several options available to distribute the potential transpiration to the different soil layers. The default option is to distribute the potential transpiration according to the root length in the different soil layers, which is a common approach in many agro-ecological models. The root length in the different soil layers is calculated by the component [TRootGrowth](https://agronomykiel.github.io/HUME/Components/RootGrowth/Documentation/TRootGrowth.html) and is provided as an external value to the component TSoilWaterModelR.

The potential sink term is then calculated by the root length in the layer cm cm^-2^ (RL~\[i\]~), an exponential factor for root competition \[-\] (CompF) divided by the sum of root length in all layers \[cm cm^-2^\] (RL~\[j\]~) and CompF (@fig-CompF). The factor 0.1 is the conversion from cm to mm which is used to stay with the units of mm for the evapotranspiration variables and the default unit of the Soil processes which is cm.
