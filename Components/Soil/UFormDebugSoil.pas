...
uses
  ...
  vcl.Dialogs, VCLTee.Chart, VCLTee.Series, VCLTee.TeEngine, vcl.ExtCtrls, VCLTee.TeeProcs, URootedSoil, UFormDebugAbstract,
...
type
  ...
  published
    property SoilWaterModel : TSoilWaterModelR read fSoilwaterModel write setSoilwaterModel;
...
//          FormDebugSoil.Visible := false;
  ...
  FormDebugSoil.SeriesSoilWater := TPointSeries.create(FormDebugSoil.ChartDebugSoil);
  ...
  FormDebugSoil.SeriesTension := TPointSeries.create(FormDebugSoil.ChartTension);
  FormDebugSoil.SeriesNewTension := TPointSeries.create(FormDebugSoil.ChartTension);
...
begin
 LabelIniFileName.Caption := ExtractFileName(SoilWaterModel.GlobMod.ActIniFile.FileName);
  ...
  for I := 1 to n_comp do begin
    with fsoilwatermodel do begin
      FormDebugSoil.SeriesFK.AddXY(FK_arr[i], -Depth[i].v+Thick[i]/2, '', ClTeeColor);
      FormDebugSoil.Seriesbsat.AddXY(Wpar[i].b_sat, -Depth[i].v+Thick[i]/2, '', ClTeeColor);
      FormDebugSoil.SeriesPWP.AddXY(PWP_arr[i], -Depth[i].v+Thick[i]/2, '', ClTeecolor);
    end;
  end;
...
//  FormDebugSoil.Series1.clear;
  ...
  with fsoilwatermodel do begin
      FormDebugSoil.SeriesSoilWater.AddXY(theta_arr[i].v, -Depth[i].v+Thick[i]/2, '', ClTeeColor);
      FormDebugSoil.SeriesFlow.AddXY(WflowInt_arr[i].v , -Depth[i-1].v, '', ClTeeColor);
      FormDebugSoil.SeriesSink.AddXY(Sink_arr[i].v*10 , -Depth[i].v+Thick[i]/2, '', ClTeeColor);
//      FormDebugSoil.SeriesSink.AddXY(fSoilWatermodel.cumBilanz_f_arr[i]*100 , -Tiefe[i].v+Dicke[i]/2, '', ClTeeColor);
      FormDebugSoil.SeriesTension.AddXY(psi_arr[i].v , -Depth[i].v+Thick[i]/2, '', ClTeeColor);
      FormDebugSoil.SeriesNewTension.AddXY( psi_neu[i] , -Depth[i].v+Thick[i]/2, '', ClTeeColor);
  end;
  end;
  self.LabelSimTime.Caption := 'Date: ' + DateTimeToStr((fSoilwatermodel.GlobTime.v));
  self.LabelBilanz1.Caption := 'Bilanz [mm]: ' + FloatToStrf(fSoilwatermodel.CumWaterBalance.v, ffgeneral, 5,2);
  self.LabelRain.Caption := 'Rain [mm]: ' + FloatToStrf(fSoilwatermodel.NetRain.v, ffgeneral, 5,2);
  self.Label_dt_int.Caption := 'dt_int [d]: ' + FloatToStrf(fSoilwatermodel.dt.v, ffgeneral, 5,2);
  self.LabelIterInt.Caption := 'IterInt: ' + FloatToStrf(fSoilwatermodel.iter, ffgeneral, 5,0);
  self.LabelIntBilFehler.Caption := 'Int. Bilanz [mm]: ' + FloatToStrf(fSoilwatermodel.sum_Bilanz_f*10, ffgeneral, 5,2);
  self.LabelGlobTime.Caption := 'Time: ' + FloatToStrf(fSoilwatermodel.SumOfInternalTimeSteps.v, ffgeneral, 12,4);
