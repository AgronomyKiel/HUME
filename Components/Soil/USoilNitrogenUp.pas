...
type
  ...
  public
    ...
    NInflux_WL: TVar;       /// nitrate influx per unit root length
    NInflux_WL_eff: TVar;   /// nitrate influx per unit root effective length
    ...
    not_av_N       : TPar;  /// not plant available part of nitrate [kg N/10 cm]
    RootRad        : TPar;  /// root radius [cm]
    Max_Wl_Nuptake : TPar;  /// maximum value for N uptake per unit root length [kg/cm/d]
    // NTotal       : TExternV;
    Ex_PlantNDemand: TExternV;  /// plant N demand [g N/m²/d]
  ...
  if Exwld_arr[1].v > 0.0 then begin // Sind Wurzeln da ?
    for i := 1 to Max_Root_Index do begin
      if (Exwld_arr[i].v * Thick[i] > 1E-4) then begin
        ...
        Imax_arr[i] := min(max_Wl_NupTake.v,max(0, Imax(NConc[i].v, Cl_min_arr[i], theta_arr[i].v, WInflux[i],
...
//        if (Max_uptake[i]/dt.v > (NitrateAmount[i].v))  then
        if (Max_uptake[i] > (NitrateAmount[i].v))  then
          max_uptake[i] := 0.5 * NitrateAmount[i].v ;//*dt.v; 09.09.2021 hk sollte auch so stabil laufen
...
begin
  ...
  fplantNDemand := max(0, Ex_PlantNDemand.v * 10); // Umrechnung von gN/m2 auf kg N/ha
