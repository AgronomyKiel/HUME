...
type
  ...
  public
    ...
    SpecWeight, ClayContent, GPV: TPar;
    Theta: TSoilExtArray;
  ...
  published
    ...
    property SoilWaterModel: TSoilWaterMod read fSoilWaterModel
      write fSoilWaterModel; // dn
...
begin
  ...
  begin
    if fSoilWaterModel.Opt_VanGenPars_from_Texture = fromTexture then
      fClay := ClayFromTexture(fSoilWaterModel.Texture[i])
    else
      fClay := ClayContent.v;
  end
  else
...
begin
  ...
  ExternVCreate('TMPM', '[°C]', StateField, AirTemp);
...
begin
  ...
  b_vektor[1] := Temp[1].v - Temp[0].v * Lambda[1].v * K /
    (Depth[0].v - Depth[1].v);
  diag[1] := -Lambda[1].v * K / (Depth[0].v - Depth[1].v) - Lambda[1].v * K /
    (Depth[1].v - Depth[2].v) + 1;
...
begin
  ...
  begin
    ...
    diag[i] := -Lambda[i].v * K / (Depth[i - 1].v - Depth[i].v) - Lambda[i].v *
      K / (Depth[i].v - Depth[i + 1].v) + 1;
...
begin
  ...
  lower[n_comp] := Lambda[n_comp].v * K /
    (Depth[n_comp - 1].v - Depth[n_comp].v);
  diag[n_comp] := -Lambda[n_comp].v * K /
    (Depth[n_comp - 1].v - Depth[n_comp].v) + 1;
end;
...
begin
  ...
  For i := 1 to n_comp + 1 do
  begin
    Lambda[i].v := lambda_f(Theta[i].v, SpecWeight.v, fClay);
...
begin
  ...
  MiddleLayers;
  LowerBoundary;
  trdiag(false, n_comp, 1, lower, diag, upper, b_vektor);
