...
type
  ...
  protected
    ...
    procedure CalcWGs; override;
    // Berechnung der abgeleiteten Wassergehalte in verschiedenen Horizonten
    function getPsiRoot: extended; override;
    procedure UpdateWGs;
    // liest WG aus externen Variablen (wenn OptUseExternalWG=true)
  public
    FTransCalcOption: TOption;
    FUseExternalWGOption: TOption;
    Wld_arr: TSoilExtArray; // Wurzellängendichten [cm.cm-3]
    ExternWG_Arr: TSoilExtArray;
    // Wassergehalte als externe Varialen eingelesen (wenn OptUseExternalWG=true)
    ...
    CumWaterUptake: TState;
    // kumulierte über die Wurzeln aufgenommene Wassermenge [mm]
    PotTrans: TVar; // potentielle Transpirationsrate [mm]
    PotTrans_: TVar;
    // potentielle Transpirationsrate im internen Zeitschritt [mm]
    ...
    delta_T: TVar;
    // Temperaturdifferenz Bestandestemperatur minus Lufttemperatur [°C]
    ...
    procedure writeValues(FirstTime: boolean; s: string); override;
    { temporäre Ausgabe-Funktion, nur für Entwicklung }
    procedure writeDebug(Schicht: integer; dt, InFlow, OutFlow, Sink, Thick,
      WGalt, WGneu, Bil: real); override;
  ...
  published
    ...
    property OptTransCalcMethod: TTransCalcMethod read FTransCalcMethod
      write FTransCalcMethod;
    property OptUseExternalWG: boolean read FUseExternalWG
      write setUseExternalWG;
...
begin
  inherited CreateAll;
  SetEntityDeclarationClass(TSoilWaterETR);
  ...
  VarCreate('delta_T', '[°C]', 0.0, false, delta_T,
    'Bestandestemperatur-Lufttemperatur');
  VarCreate('Tbase', '[°C]', 0.0, false, Tbase,
    'theoretische min. Bestandestemperatur');
  VarCreate('Tmax', '[°C]', 0.0, false, Tmax,
    'theoretische max. Bestandestemperatur');
  StateCreate('CumActTrans', '[mm]', 0, true, CumActTrans);
  StateCreate('CumPotTrans', '[mm]', 0, true, CumPotTrans);
  StateCreate('CumWaterUptake', '[mm]', 0, true, CumWaterUptake,
    'kumulierte über die Wurzeln aufgenommene Wassermenge [mm]');
  ...
  OptCreate('UseExternalWG', 'true', FUseExternalWGOption,
    'If true then WGs are loaded from weatherfile each time step, else internal calculated WGs are used');
  ...
  for i := 1 to n_comp do
    VarCreate('K_' + IntToStr(i), '[cm/d]', 0.0, true, K_arr[i]);

  if FWithRoots then
    for i := 1 to n_comp do
    begin
      ExternVCreate('effWLD_' + IntToStr(i), '[cm/cm3]', StateField,
        Wld_arr[i]);
  ...
  if OptUseExternalWG then
    ...
    begin
      ExternVCreate('WG' + IntToStr(i), '[cm3/cm3]', StateField,
        ExternWG_Arr[i]);
...
begin
  ...
  for i := 1 to n_comp do
    Sink_arr[i].v := 0.0;
  If uppercase(FTransCalcOption.Option) = 'REDFACTOR' then
    FTransCalcMethod := redFactor;
  If uppercase(FTransCalcOption.Option) = 'RESISTANCES' then
    FTransCalcMethod := resistances;
  if FUseExternalWGOption.Option = 'true' then
    OptUseExternalWG := true;
  if FUseExternalWGOption.Option = 'false' then
    OptUseExternalWG := false;
  if FDebug then
    writeValues(true, 'Init');
  for i := 1 to n_comp do
  begin
...
function TSoilWaterETR.Trans_f(psi_leaf: real): real;
{ Transpirationsrate [cm/d] }
const
  a = 0.02; { mittlerer Wurzeldurchmesser [cm] }
  Rrr = 0.25E6;
  { radiale Wurzel-"resistivity"  [d/cm] umgerechnet aus Reid (1991) }
...
begin
  ...
  begin
    ...
    begin
      ...
      ri_arr[i].v := r_interf_f(Rrr, thetaadj_arr[i].v, WPar[i].b_sat,
        Wld_arr[i].v, Thick[i]);
      Sink_arr[i].v :=
        max(((psi_leaf + cropheight.v * 100) - (psi_arr[i].v + Depth[i].v - 0.5
        * Thick[i])) / (rs_arr[i].v + ri_arr[i].v + r_stem.v), 0);
      { Senkenstärke [cm/d] }
      ...
      resistances:
        begin
          ...
          begin
            psi_leaf.v := 1000;
            for i := 1 to n_comp do
              if Wld_arr[i].v > 0 then
              begin
                if psi_arr[i].v > psi_leaf.v then
                  psi_leaf.v := psi_arr[i].v;
              end;
            // int_stor_.v := int_stor.v;
            CalcActET;
            If (Wld_arr[1].v <= 0.0) or (ActTrans_.v <= 0.0) then
            begin
              for i := 1 to n_comp do
                Sink_arr[i].v := 0;
              { intWflow_arr[1].v := -0.1*ActEvap_.v/GlobMod.TimeStep + 0.1*NetRain.v/GlobMod.TimeStep;
            ...
            begin
              ...
              repeat
                ...
                if abs(f) > max_TransErr.v then
                begin
                  uptake2 := Trans_f(psi_leaf.v + delta_psi) * GlobMod.TimeStep;
                  f2 := ActTrans_.v / 10.0 - uptake2;
                  df := (f - f2) / delta_psi;
                  if abs(df) < 0.000000001 then
                  begin
                    success := true;
                    break;
                  end;
                  while abs(df) < 0.0001 do
                    df := df * 10;
                  if (psi_leaf.v = 0) and (f / df <= 0) then
                  begin
                    success := true;
                    break;
                  end;
                  psi_leaf.v := psi_leaf.v + f / df;
                  if psi_leaf.v < 0 then
                    psi_leaf.v := 0;
                end
                else
                  success := true;
          ...
          intWflow_arr[1].v := -0.1 * ActEvap_.v / GlobMod.TimeStep + 0.1 *
            NetRain.v / GlobMod.TimeStep;
          CheckSinks;
          if (Interzeption.v > 0) and
            (Interzeption.v > (aETP.v - (aETP.v * f_Evap) - ActTrans_.v)) then
          begin
            int_stor.v := int_stor.v + Interzeption.v -
              (aETP.v - (aETP.v * f_Evap) - ActTrans_.v);
            Interzeption.v := (aETP.v - (aETP.v * f_Evap) - ActTrans_.v);
          end;
        ...
        begin
          ...
          begin
            if Sqr_Wl_arr[i] > 1E-6 then
              Sink_arr[i].v := 0.1 * pot_trans.v * Sqr_Wl_arr[i] / Sum_Sqr_wl
            else
              Sink_arr[i].v := 0.0;
...
begin
  ...
  gamma := cp * pressure / (0.622 * (2.502 * 1000000 - 2361 * TMPM.v));
  // statt gamma := Pressure*Psycro :  (dn 10.06.14)
  ...
  if cropheight.v <= 0.0 then
    ra.v := ra_f(wind_speed.v, 0.05)
  else
    ra.v := ra_f(wind_speed.v, cropheight.v);
  pETP.v := Penman(TMPM.v, Sat_def.v, netRad.v, delta, gamma, l_h_v_water, ra.v,
    rc_pot) * GlobMod.Time.c;
  if LAI.v > 0.0 then
    aETP.v := Penman(TMPM.v, Sat_def.v, netRad.v, delta, gamma, l_h_v_water,
      ra.v, rc_f) * GlobMod.Time.c
  else
    aETP.v := pETP.v;
...
begin
  ...
  // Berechnung der Nettostrahlung nach empirischer Funktion
  // gefitted aus Tagesdaten gemessener Nettostrahlung (W/m2) zu Globalstrahlung (W/m2)
  netRad.v := 0.6494 * (GlobRad.v) - 18.417;
  // max(0,0.6494*(GlobRad.v) - 18.417);
  es := sat_vap_press_f(TMPM.v);
  delta := delta_f(es, TMPM.v);
  if cropheight.v <= 0.0 then
    ra.v := ra_f(wind_speed.v, 0.05)
...
begin
  if (FWithRoots = true) and (FTransCalcMethod = redFactor) then
    Calcsink_red_f;
  inherited CalcRatesAndIntegrate;
  if (FTransCalcMethod = redFactor) then
...
const
  cp = 1003.0; { spezifische W„rme der Luft [J/(Kg*K)] }
  Imax = 0.8;
  { nach Jackson et al. 1988, wegen erhöhter Reflexion und Emission des fiktiv nicht transpirierenden Bestandes }
  Ibase = 0.9;
  { nach Jackson et al. 1988, Berücksichtigung des Bodenwärmestroms }
begin
  ...
  if FDebug then
    writeValues(false, 'CalcRates vor Schleife');
  CalcPotET;
  ...
  repeat
    ...
    for i := 1 to n_comp + 1 do
      Wflow_arr[i].v := Wflow_arr[i].v + intWflow_arr[i].v /
        GlobMod.Time.c * dt.v;
    ...
    if FDebug then
      writeValues(false, 'CalcRates in Schleife');
  ...
  deltaTbase := ra.v * Ibase * netRad.v / (cp * rho) * gamma / (delta + gamma) -
    Sat_def.v / (delta + gamma);

  Tmax.v := TMPM.v + deltaTmax;
  Tbase.v := TMPM.v + deltaTbase;

  if (PotTrans.v + Pot_Evap.v) <> 0 then
    CWSI.v := 1 - (ActTrans.v + Act_Evap.v) / (PotTrans.v + Pot_Evap.v)
  else
    CWSI.v := 0;
  if ActTrans.v = 0 then
    CWSI.v := 0;
...
begin
  ...
  begin
    ...
    for i := 1 to 20 do
      write(f, ';thetaadj_arr', i);
    for i := 1 to 20 do
      write(f, ';WFlow_arr', i, ';intWFlow_arr', i);
  end
  else
  begin
    Write(f, TimeToStr(Time), ';', GlobTime.v, ';', dt.v, ';', ActTrans.v, ';',
      ActTrans_.v, ';', PotTrans.v, ';', PotTrans_.v);
    for i := 1 to 20 do
      write(f, ';', thetaadj_arr[i].v);
    for i := 1 to 20 do
      write(f, ';', Wflow_arr[i].v, ';', intWflow_arr[i].v);
  end;
  writeln(f, ';', s);
  CloseFile(f);
end;

procedure TSoilWaterETR.writeDebug(Schicht: integer;
  dt, InFlow, OutFlow, Sink, Thick, WGalt, WGneu, Bil: real);
{ temporäre Ausgabe-Funktion, nur für Entwicklung }
var
  f: TextFile;
  i: integer;
...
begin
  ...
  writeln(f, TimeToStr(Time), ';', Schicht, ';', dt, ';', InFlow, ';', OutFlow,
    ';', Sink, ';', Thick, ';', WGalt, ';', WGneu, ';', Bil);
  CloseFile(f);
end;
...
begin
  inherited Integrate;
  If (ActTrans.v > 0) and (PotTrans.v > 0) then
