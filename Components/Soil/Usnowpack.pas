...
Type
  ...
  public
    ...
    rhop: TVar;
    // density of newly fallen precipitation (snow-rain mixture), calculated as a weighted average of the density of water and powder snow
    ScS: TVar;
    // storage capacity on snow storage for retention of liquid water [mm]
    Si: TVar; // radiation
  ...
  published
    ...
    Property Ex_global_radiation: TExternV read global_radiation
      write global_radiation;
...
begin
  ...
  ParCreate('alpha_h', '[W m4 kg-2]', 2.86E-6, alpha_h,
    'Thermal conductivity parameter for snow');
...
begin
  ...
  If Tact.v <= t1.v then
    Ps.v := P.v
  else If t2.v <= Tact.v then
    Ps.v := 0
  else
    Ps.v := ((t2.v - Tact.v) / (t2.v - t1.v)) * P.v;
  // Unterscheidung Schnee oder Regen in Abhägikeit der Temperatur
  ...
  If SwS.v <= 0 then
    Es.v := 0
  else If Es.v > P.v + SwS.v then
    Es.v := P.v + SwS.v
  else
    Es.v := Epot.v; // Berechnung Evaporation + Sublimation
  If Es.v < 0 then
    Es.v := 0;
  If Es.v > Epot.v then
    Es.v := Epot.v;
  If Es.v > SsS.v + P.v then
    Es.v := SsS.v + P.v;

  If SwS.v <= 0 then
    EsS.v := 0
  else If Es.v > Pr.v + SsS.v then
    EsS.v := Pr.v + SwS.v
  else
    EsS.v := Es.v; // Berechung Evaporation

  Zs.v := SsS.v / rhos.v;
  // Schichtdicke vom Vortag, neuer Schnee kommt bei mt dazu
  ...
  If SwS.v <= 0 then
    ...
    maxM.v := ((SsS.v - SwS.v) + Ps.v - (Es.v - EsS.v));

  If SwS.v <= 0 then
    minM.v := 0
  else
    minM.v := -(SwS.v + Pr.v - EsS.v);
  ...
  If SwS.v <= 0 then
    ...
    Js.v := max(0, SwS.v + (Pr.v - Es.v + M.v) - ScS.v);
  ...
  if SwS.v > 0 then
    ...
    Tsf.v := ((Lambda1.v / z1.v) * Temp1.v + alpha_h.v * sqr(rhos.v) / Zs.v *
      AirTemp.v) / (Lambda1.v / z1.v + alpha_h.v * sqr(rhos.v) / Zs.v)
  else
    Tsf.v := AirTemp.v;
