...
uses vcl.Graphics, Classes, GraphObjects;
...
begin
  ...
  e := chain.Items[0];
  if e.ey > 0 then // put v into a "conveniant large ordinate"
    v := TGPoint.Create(e.p2.getx + 6000 * e.ex,
      e.p2.gety + 6000 * e.ey, nil, nil)
  else
    v := TGPoint.Create(e.p2.getx - 6000 * e.ex, e.p2.gety - 6000 * e.ey,
      nil, nil);
  ...
  repeat
    ...
    begin
      ...
      if ((aex < bex) and (aex >= 0)) or ((aex > bex) and (aex < 0)) then
        TGLine(el.Items[i1]).CutLeft(e)
      else
        TGLine(el.Items[i1]).CutRight(e);
    ...
    begin
      ...
      if ((aex < bex) and (aex >= 0)) or ((aex > bex) and (aex < 0)) then
        TGLine(er.Items[i2]).CutRight(e)
      else
        TGLine(er.Items[i2]).CutLeft(e);
    ...
    e.CutBoth(chain.last);

  done:

    e := chain.last;

  until ((e.BisectorOf[1] = TGPoint(ConvexHull1.Items[i1min]).GetOrigIndex) and
    (e.BisectorOf[2] = TGPoint(ConvexHull2.Items[i2min]).GetOrigIndex)) or
    ((e.BisectorOf[2] = TGPoint(ConvexHull1.Items[i1min]).GetOrigIndex) and
    (e.BisectorOf[1] = TGPoint(ConvexHull2.Items[i2min]).GetOrigIndex));
