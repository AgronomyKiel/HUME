...
uses
  Ustate, UlayeredSoil, UMod, IniFilesNew, classes, UAbstractSoilMin;
...
begin
  ...
  ParCreate('HumusContent', '[-]', 0.018, HumusContent, 'humus concentration');
...
begin
  ...
  case ProcessType of
    denp:
      begin
        //f_W := min(1, max(0, (WC-min(CritDen_f.v*FK,0.99*Sat))/(Sat-min(CritDen_f.v*FK,0.99*Sat))));
...
begin
  ...
  begin // set mineralisation rate in linked transport model
    ...
    for Procs := low(processes) to high(processes) do
      ...
      //CO2 := CO2FlowArr[Procs, layer].v + CO2;  hier evtl. andere Umschreibung für active V =potential CO2-flow (without W und T Einfluss) dn
...
//    NextINI.c
   ...
   for pool := Low(Pools) to High(Pools) do
      NextStateINI.WriteString(Name, CPool[pool].Name, FloatToStrF(CPool[pool].v, ffFixed, 9, 6));
...
//    NextStateINI.WriteString(Name, 'C_RPM', FloatToStrF(CPool[RPM].v, ffFixed, 9, 6));
    ...
    for i := 1 to trunc(NOrgLayers.v) do
    begin
      NextStateINI.WriteString(Name, 'NH4_Arr_' + ndx_str(i),
