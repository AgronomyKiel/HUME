unit UFormYearAnalysis_PhenTrend;
...
type
   TManagementEvents = (Tillage, Fert_NH4 , Fert_NO3, Dueng1);
...
const
  ...
  TManagementEventStrgs : array[TManagementEvents] of string = ('Tillage', 'Fert_NH4','Fert_NO3', 'Dueng1');
...
type
  ...
  TFormYearAnalysis = class(TForm)
    ...
    LabelSowingdatetrend: TLabel;
    EditSowingDateTrend: TEdit;
...
//    self.DateTimePickerSowingDate.MinDate := FirstWeatherData;
    ...
    self.LabelSowingdatetrend.Visible := true;
    self.EditSowingDateTrend.Visible := true;
    WeatherFile.GoTop;
  end;
  TempString.Free;
...
//  OpenDialog1.Filter := ' (*.*)|*.*';
  ...
  begin
    ...
    self.LabelFirstWeather.Caption := DateToStr(Firstweatherdata);
    self.LabelFirstWeather.Visible := true;
 //   self.DateTimePickerStartDate.MinDate := StrToDate(FloatToStr(FirstWeatherData));
    DateTimePickerStartDate.visible := true;
    self.LabelStartDate.Visible := true;
 //   self.DateTimePickerSowingDate.MinDate := FirstWeatherData;
    DateTimePickerSowingDate.visible := true;
...
var
  ...
  s: string;
  NewDate, DiffDay : integer;
  TemplateInputFile, NewInputFile: TextFile;
  sowingdatetrend : real;
...
{  for I := 0 to model.SubModStrList.Count - 1 do begin
  ...
  sowingdatetrend := StrToFloat(self.EditSowingDateTrend.Text);
  ...
  // Create and modify ParIniFiles
    ...
    for i := 0 to SubModelStrings.Count - 1 do begin
      ...
      if  Index <> -1 then
         ...
         DiffDay :=  round(sowingdatetrend*(year-StartYear));
        NewDate  := NewDate+DiffDay;
