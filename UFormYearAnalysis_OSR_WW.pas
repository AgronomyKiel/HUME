...
type
  ...
  cfn,   test,
  sitename : string;
  TemplateIniFile, NewIniFile, TemplateParIniFile, TemplateStateIniFile, NewParIniFile, NewStateIniFile, ParIniFile : TMyIniFile;
  SowingdatePar : TPar;
  actSubmod : TSubmodel;
...
//    self.WeatherFile
  ...
  for i := 0 to SubModelStrings.Count - 1 do begin
    ...
    if Index <> -1 then begin
      OldCropHarvestDates[i] := TemplateParIniFile.Readfloat(SubmodelStrings[i], HarvestDateString, NewCropSimDates[Harvest]);
  ...
  if FileExists(TemplateStateIniFileName) then begin
    ...
    Index := Submodelstrings.IndexOf(SectionNameTillage);

    for I := low(TManagementEvents) to high(TManagementEvents) do begin
      TemplateStateIniFile.ReadSection(TManagementEventStrgs[i], StateNames);
      for j := 0 to StateNames.Count - 1 do begin
        self.ManagementDates[i,j] := strtoDate(StateNames[j]);
        self.ManagementValues[i,j] := TemplateSTateIniFile.readfloat(TManagementEventStrgs[i], StateNames[j]);
      end;
    end;
...
//    NewIniFile.UpdateFile;
  // Create and modify ParIniFiles
    ...
    for i := 0 to SubModelStrings.Count - 1 do begin
      ...
      Index := Parnames.Indexof(SowingDateString);
      if  Index <> -1 then
        NewParIniFile.WriteInteger(SubModelStrings[i], SowingDateString, trunc(OldCropSowingDates[i]));
