...
type
   TCropSimDates = (SimStart, SimEnd, Sowing, Harvest);
  ...
  cfn,   test,
  sitename : string;
  TemplateIniFile, NewIniFile, TemplateParIniFile, NewParIniFile, ParIniFile : TMyIniFile;
  SowingdatePar : TPar;
  actSubmod : TSubmodel;
...
var
  ...
  i, index : integer;
  Date : TDate;
  SubModelStrings, ParNames,  TempString : TStringlist;
begin
...
//    self.WeatherFile
  ...
  for i := 0 to SubModelStrings.Count - 1 do begin
    ...
    if  Index <> -1 then begin
      date := TemplateParIniFile.Readfloat(SubmodelStrings[i], SowingDateString, 0);
      DateTimePickerSowingDate.DateTime := Date;
    end;

    Index := Parnames.Indexof(HarvestDatestring);
    if Index <> -1 then begin
      date := TemplateParIniFile.Readfloat(SubmodelStrings[i], HarvestDateString, NewCropSimDates[Harvest]);
      self.DateTimePickerHarvestDate.DateTime := Date;
    end;
   end;
...
var
  i, index, year : integer;
  CropSimDates  : TCropSimDates;
  SubModelStrings, ParNames : TStringlist;
...
//  NewSowingdate
  ...
  NewCropSimDates[Sowing] := round(DateTimePickerSowingDate.DateTime);
  NewCropSimDates[Harvest] := round(DateTimePickerHarvestDate.DateTime);
...
//  closefile(Controlfile);
  ...
  NewCropSimDates[Sowing] := self.DateTimePickerSowingDate.DateTime;
  NewCropSimDates[Harvest] := self.DateTimePickerHarvestDate.DateTime;
...
//    NewIniFile.UpdateFile;
  // Create and modify ParIniFiles
    ...
    for i := 0 to SubModelStrings.Count - 1 do begin
      ...
      Index := Parnames.Indexof(SowingDateString);
      if  Index <> -1 then
        NewParIniFile.WriteInteger(SubModelStrings[i], SowingDateString, trunc(NewCropSimDates[Sowing]));

      Index := Parnames.Indexof(HarvestDateString);
      if Index <> -1 then
        NewParIniFile.WriteInteger(SubModelStrings[i], HarvestDateString, trunc(NewCropSimDates[Harvest]));
    end;
