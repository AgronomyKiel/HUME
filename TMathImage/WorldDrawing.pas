...
uses Windows, Classes, vcl.Graphics, SysUtils,
{$IFDEF MATH}
  math
{$ENDIF};
...
function max(x, y: MathFloat): MathFloat; overload;
begin
  Result := x;
  if y > x then
    Result := y;
...
function min(x, y: MathFloat): MathFloat; overload;
begin
  Result := x;
  if y < x then
    Result := y;
...
begin {******* drawd3axes ******}
  ...
  begin
    ...
    begin
      ...
      If Ticks <= 0 then ticks := 1; //hk 3.9.05 
      i := iStart;
      if Ticks <= 500 then
        repeat
