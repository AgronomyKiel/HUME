...
uses
  SysUtils, WinProcs, WinTypes, Messages, Classes, vcl.Graphics,
  vcl.Controls, vcl.Forms, vcl.Dialogs, OverlayImage, WorldDrawing;
...
{: <New topic=Overview@Overview>
   ...
   <LI=<See method=TMathImage@DrawLevelCurves> >
...
{: ESurfaceError is raised whenever a value is being assigned to a
   nonexisting  <see class=TSurface> grid point, a color is being
   assigned to a nonexisting <see class=TColorSurface> gridpoint,
   or the corresponding are tried to be accessed.
}
...
{: The Error property has been kept for backwards compatability. Its value is
   always false now. Unless you create a surface with xGrid<=0 or yGrid<=0,
   which would be very unlogical, it will be created OK. Another cause of
   error could be an out of memory because of too many grid points. I leave it
   to Delphi to trap those errors.
}
...
{: TSurface descendent which can also store different colors.
  ...
  public
    constructor Create(xGrid, yGrid: Integer);
    destructor Destroy; override;
{: Assign the point (x,y,z) to grid location (i,j), and specify the
color for this surface part.
}
...
{:Use to read the number of points currently in the list. Note: not necessarily the same as
length(Points)!
}
...
{:D3FloatPointListList object: list of D3FloatPointLists. Intended to be passed to
  ...
  private
    ...
    procedure Setx1d2(x: MathFloat);
    procedure Setxwd2(x: MathFloat);
    procedure Sety1d2(x: MathFloat);
    procedure Setywd2(x: MathFloat);
    procedure Setx1d3(x: MathFloat);
    procedure Sety1d3(x: MathFloat);
    procedure Setxwd3(x: MathFloat);
    procedure Setywd3(x: MathFloat);
    procedure Setz1d3(x: MathFloat);
    procedure Setzwd3(x: MathFloat);
    procedure Setvd(x: MathFloat);
    procedure Setzrd3(x: MathFloat);
    procedure Setyrd3(x: MathFloat);
    procedure Setalpha(x: MathFloat);
    procedure Setard3(x: Boolean);
    procedure SetXscale(x: MathFloat);
    procedure SetYscale(x: MathFloat);
    procedure SetZscale(x: MathFloat);
    procedure SetClipRect(Value: TRect);
    procedure SetRecordMetafile(x: Boolean);
    function Getd2Worldx2: MathFloat;
    function Getd2Worldy2: MathFloat;
    function Getd3Worldx2: MathFloat;
    function Getd3Worldy2: MathFloat;
    function Getd3Worldz2: MathFloat;
    function GetAxis: Boolean;
    function Getx1d2: MathFloat;
    function Getxwd2: MathFloat;
    function Gety1d2: MathFloat;
    function Getywd2: MathFloat;
    function Getx1d3: MathFloat;
    function Gety1d3: MathFloat;
    function Getxwd3: MathFloat;
    function Getywd3: MathFloat;
    function Getz1d3: MathFloat;
    function Getzwd3: MathFloat;
    function Getvd: MathFloat;
    function Getzrd3: MathFloat;
    function Getyrd3: MathFloat;
    function Getalpha: MathFloat;
    function Getard3: Boolean;
    function GetXscale: MathFloat;
    function GetYscale: MathFloat;
    function GetZscale: MathFloat;
...
{---------------------*********************************--------------------------}
  public
    ...
    {: Intended to be able to set the current clip rectangle. Not really
    implemented so far, except for clipping the region within axes.
    }
   ...
   {:
  Draws axes centered at (xmin,ymin,zmin) extending to
(xmax,ymax,zmax),  without ticks.
  }
    procedure d3DrawCustomAxes(
    ...
    for the z-level passed in level. Note that you can pass a TSurface or
    any descendent.
    }
  ...
  published
    ...
    property RecordMetafile: Boolean read FRecordMetafile write
      SetRecordMetafile;
...
begin
  ...
  fCurrentpoint := nil;
  fCount := 0; fLength := 0;
  //not really necessary. But the following helps a bit
  //under Win2K:
  if Win32Platform = VER_PLATFORM_WIN32_NT then
    SetProcessWorkingSetSize(GetCurrentProcess, DWORD(-1), DWORD(-1));
  inherited Destroy;
...
procedure TSurfaceCollection.add(const Surface: TSurface; FillColor,
  WireColor: TColor);
