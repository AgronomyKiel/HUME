...
begin
  ...
  self.Chart1.SaveToMetafileEnh(fn);
...
begin
  FPropIniFile.WriteInteger('ComboBoxes', ComboBoxSubMod.Name, self.ComboBoxSubMod.ItemIndex);
  FPropIniFile.WriteInteger('ComboBoxes', self.ComboBoxIniFile.Name, self.ComboBoxIniFile.ItemIndex);
  FPropIniFile.UpdateFile;
  self.FPropIniFile.Free;
  img_help.Free;
  img_savetoall.Free;
...
var
  CtrlFileFN, fn, path, FirstIniFileFN, OutDir : string;
  CtrlFile : textfile;
...
begin
  ...
  fn := FPropIniFile.ReadString('Files', 'ControlFile', path+'Control.fn');
  ...
  FPropIniFile.ReadInteger('ComboBoxes', ComboBoxSubMod.Name,ComboBoxSubMod.ItemIndex);
  FPropIniFile.ReadInteger('ComboBoxes', ComboBoxIniFile.Name,ComboBoxIniFile.ItemIndex);
  ...
  CtrlFileFN := FPropIniFile.ReadString('Files', 'ControlFile', '');
...
begin
  ...
  begin
    ...
    Clear;
    Rows[0].commatext :=
      'Name, Unit, Ini.Value,, WriteToFile, Plot, WriteFinalValue,';
    // Description';
    RowCount := 2;
    FixedRows := 1;
    actSubModIndex := ComboBoxSubMod.ItemIndex;
...
begin
  ...
  begin
    BeginUpdate;
    Clear;
    Rows[0].commatext := 'Name, Unit, WriteToFile,  Plot, WriteFinalValue,';
    // Description';
    RowCount := 2;
    FixedRows := 1;
    actSubModIndex := ComboBoxSubMod.ItemIndex;
...
begin
  ...
  begin
    ...
    if SubModel.WriteContinuouslyToFile then begin
    ...
    if FileExists(SubModel.fn_state) and (SubModel.WriteContinuouslyToFile) then
    begin
      n_lineSeries := -1;
    ...
    begin
      ...
      begin
        ...
        if not ActState.writeToFile then
          GetCheckBoxState(5, i, ActState.writeToFile);
    ...
    begin
      ...
      begin
        ...
        if not ActVar.writeToFile then
          GetCheckBoxState(3, i, ActVar.writeToFile);
...
begin
  Chart1.BottomAxis.LabelsFont.Size := Chart1.BottomAxis.LabelsFont.Size + 1;
  Chart1.LeftAxis.LabelsFont.Size := Chart1.LeftAxis.LabelsFont.Size + 1;
  Chart1.Legend.font.Size := Chart1.Legend.font.Size + 1;
  Chart1.Repaint;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FPropIniFile.WriteString('Files', 'ControlFile', NewCtrlFN);
...
begin
  ...
  begin
    ...
    with ActSubMod do
    begin
      ActSubMod.WriteContinuouslyToFile := false;
...
var
  ...
  begin
    ...
    begin
      entity := THumeNumEntity(strList.objects[i]);
      with entity, FPropIniFile do
      begin
        PlotToGraph := ReadBool(submodname, Name + '.PlotTograpH', PlotToGraph);
        WriteFinalValue := ReadBool(submodname, Name + '.WriteFinalValue',
          WriteFinalValue);
        writeToFile := ReadBool(submodname, Name + '.WriteToFile', writeToFile);
...
begin
  ...
  begin
   ...
   ComboBoxSubMod.ItemIndex := FPropInifile.ReadInteger('ComboBoxes', ComboBoxSubMod.Name, 1);
