...
type
 // The following types are used as elements each submodel may contain
  TModelElements = (States, Vars, Params, Externals, Consts);   /// enumeration type for model Elements

 // for code generation and documentation
  TModelElementNames = array [TModelElements] of string;  /// enumeration type strings for model Elements

  // array for numbers of Model Elements in each submodel
  TNumbersOf = array [TModelElements] of Integer;  /// type for numbers of Model Elements in each submodel

 // for code generation and documentation
  TListsOf = array [TModelElements] of TStringList;  /// type for list of stringlists of Model Elements in each submodel

 // formerly TMyInifile was used for handling INI files
 // which is now replaced by TMemIniFile
  TMyIniFile   = TMemIniFile;  /// Use TMemIniFile for Inifiles

 // the floating point type is redefined as double
  real = double;  /// redifined floating point type


 // shape typpes for drawing of model elements todo: move to UFormMod.pas
  TShapeType = (stRectangle, stSquare, stRoundRect, stRoundSquare, stEllipse,
    stCircle);

  TWeightOptions = (OptNoWeight, OptDefaultWeight, OptMeasErrorWeight);  /// Options for weighting of data points during optimization
  TOptOption = (optAllInis, optAllInisSeperate, optOnlyActIni);
  /// Options for parameter Optimization

const
// string array for model element names
  ModelElementNames: TModelElementNames = ('State Variables', 'Variables',
    'Parameters', 'Exernal Values', 'Constants');   /// strings for Model element names


// string constant for path separator
// not yet used, instead compiler directives for different OS are used
  Path_sep =  '/';


type
  TMod = class;  /// abstract base type for a model component
// class for the coordination of the simulation of a model
// and parameter estimation etc..
 { * -----------------------------------------------------------------
    CLASS     TMod
    ANCESTOR  TGraphicControl or TObject if nonvisual
...
// abstract base class for a model component
  ...
  { * -----------------------------------------------------------------
    CLASS     TMarquardOptions
    ANCESTOR  TPersistent if visual or TObject if nonvisual
    PURPOSE   Options for the Marquard-Method used for parameter estimation
    ------------------------------------------------------------------ }
/// <summary>
/// TMarquardOptions is a class that represents the options for the Marquard optimization algorithm.
/// </summary>
TMarquardOptions = class(TPersistent)
  ...
  public
    ...
    FDivisor: real;   /// Value which is needed for numerical approximation of function derivative
    FWeightOptions: TWeightOptions;   /// Options for weighting of data points during optimization
  ...
  { * ----------------------------------------------------------------
    ...
    ANCESTOR  TPersistent or TObject if nonvisual
    PURPOSE   Simple class to save options for Sensitivity analysis
    used within the TMod class
    ------------------------------------------------------------------ }
{$IFDEF NONVISUAL}
  /// Represents the sensitivity options for a non-visual component.
  TSensitivityOptions = class(TObject)
{$ELSE}
  /// Represents the sensitivity options for a persistent component.
  TSensitivityOptions = class(TPersistent)
{$ENDIF}
  private
    fMod: TMod;
    f_a, f_b: array [0 .. 30] of textFile;  /// file variables and names for output
    fn_a, fn_b: array [0 .. 30] of string; /// file variables and names for output
    ...
    FDPar: real; /// step size for sensitivity analysis
    ...
    /// Sets the maximum value during sensitivity analysis.
    procedure SetMAxValue(const MaxValue: real);
    /// Sets the minimum value during sensitivity analysis.
    procedure SetMinValue(const MinValue: real);
    /// Sets the number of steps of sensitivity analysis.
  ...
  published
    /// Gets or sets the maximum value during sensitivity analysis.
    property MaxValue: real read FMaxValue write SetMAxValue;
    /// Gets or sets the minimum value during sensitivity analysis.
    property MinValue: real read FMinValue write SetMinValue;
    /// Gets or sets the number of steps of sensitivity analysis.
    property Steps: Integer read FSteps write SetSteps;
    /// Gets or sets the step size for sensitivity analysis.
    property DPar: real read FDPar write FDPar;
    /// Gets or sets the list of variables selected for output.
    property OutList: TStringList read FOutList write FOutList;
    /// Gets or sets the output file name for sensitivity data.
    property Sens_fn: string read FSens_fn write FSens_fn;
  end;

{$IFDEF NONVISUAL}

/// Implementation of class for coordinating the simulation of a model
TMod = class(TObject)
  {$ELSE}
  ...
  private
    ...
    fControlFileFn: string; /// Name for controlfile
    ...
    fChiSqr: real;     /// field for total sum of squared differences (sim-meas)
    fContOutput: boolean; /// field for toggle choice for file output every time step
    fOptContOutput: TOption;   /// TOption Object for implenting Toggle choice for file output every time step

    FStr_SectionName_TimeInit, /// Section name for time initialisation
    FStr_SectionNameMeasurementFiles, /// Section name for measurement files
    FStr_SectionNameUpdateFiles, /// Section name for update files
    FStr_SectionNameOutputFiles, /// Section name for output files

    FStr_SectionName_FileNames, /// Section name for file names
    FStr_SectionTopic_SimStart, /// Section topic for simulation start
    FStr_SectionTopic_SimEnd, /// Section topic for simulation end
    FStr_SectionName_SimOptions, /// Section name for simulation options
    FStr_SectionTopic_TimeStep, /// Section topic for time step
    FStr_SectionTopic_ContOutput, /// Section topic for continuous output
    FStr_SectionTopic_StateIniFN, /// Section topic for state initialisation file name
    FStr_SectionTopic_ParamIniFN,    /// Section topic for parameter initialisation file name
    FStr_SectionTopic_OptionIniFN,    /// Section topic for option initialisation file name
    FStr_SectionTopic_WeatherFileFN,  /// Section topic for weather file name
    FStr_SectionTopic_OutputDir       /// Section topic for output directory
    : string;
  ...
  public
    ...
    constructor create;
    {$ENDIF}
    procedure Set_ControlFileFN(NewFN:string);
    ...
    procedure CalcChiSquareSensitivity; /// Sensititvity of ChiSquare Values over 1 to several simulation runs
    ...
    property SubModel[Index: Integer]: TSubmodel read getSubModel
      write setSubModel;    /// Array property for convenient Access of Submodels
    property ReadIniOutputPath: boolean read fReadIniOutputPath write fReadIniOutputPath;

    {$IFNDEF NONVISUAL}
    procedure DblClick; override; /// show info during runtime on DblClick event
    {$ENDIF}

  published
    // property Name:string read fName write fName; /// Name of Model
    property GM_ControlFile: string read Get_ControlFileFn
      write Set_ControlFileFN; /// Name of control file
    property GM_SubModStrList: TStringList read SubModStrList
    { write SubModStrList }; /// read property with List of submodels
    property ApplicationPath: TPath read FApplicationPath
      write FApplicationPath; /// directory where program file is located
    property GM_OutPutPath: TPath read FOutputPath write FOutputPath; /// default directory for output
    property GM_InPutPath: TPath read FInputPath write FInputPath; /// default directory for input
    property GM_OutputFileName: string read fFNGlobalOutput write fFNGlobalOutput; /// file name for global output
    property Separator: Char read FSeparatorChar write FSeparatorChar default ','; /// separator in outputfiles
    property TimeStep: real read FTimeStep write Set_TimeStep; /// Time step for model integration
    property StartTime: TDateTime read FStartTime write Set_StartTime; /// Start of simulation
    property EndTime: TDateTime read FEndTime write Set_EndTime; /// End of simulation
    property ChiSqr: real read fChiSqr write fChiSqr; /// total sum of squared differences (sim-meas)
    property Reg_fn: string {TMyFileName} read FReg_FN write FReg_FN; /// Name of file where regression results are stored
    property WriteResIni: boolean read fWriteResIni write fWriteResIni; /// flag for writing res.ini files for documentation of simulation parameters
    property Docu_fn: string {TMyFileName} read fDocu_FN write fDocu_FN; /// Name of file where Model documentation is stored
    property Docu_fn2: string {TMyFileName} read fDocu_FN2 write fDocu_FN2;
    property ModTime: TState read Time write Time; /// model time as a state variable
    property LMOptions: TMarquardOptions read FLMOptions write FLMOptions; /// Options for optimization
    property SensOpt: TSensitivityOptions read FSensOptions write FSensOptions; /// Options for sensitivity analysis
    property IniFileNames: TStringList read FIniFiles; /// List of Inifiles
    property Title: string read FTitle write FTitle; /// Modeltitle
    property ContOutput: boolean read FContOutput write FContOutput; /// Toggle choice for file output every time step
    property FinalOutput: boolean read FFinalOutput write FFinalOutput; /// Toggle choice for file output at end of simulation
    property MinLegalValue: real read FMinLegalValue write FMinLegalValue;  /// smallest measurement value accepted in optimization
...
{$ENDIF}
  private
    ...
    fOptContOutput: TOption;  /// TOption object for handling private field fWritecontinuouslyToFile
    fWriteFinallyToFile: boolean;   /// Should output of the submodel be written to file finally
    fOptFinalOutput: TOption;  /// TOption object for handling handling private field fWriteFinallyToFile
 ...
 // methods for handling model elements
 ...
 // methods for initalisation model elements
...
// procedures for initialisation of files
  ...
  protected
    function Get_GlobMod: TMod; virtual; /// Get pointer to global model
    procedure Set_GlobMod(Model: TMod); virtual; /// Set pointer to global model
    function UpdateValue(n: string): real; virtual; /// Update value
   {$IFNDEF NONVISUAL}
  ...
  public
    ...
    f_fin_state: TStreamWriter; /// File variable for final state output, now implemented as TStreamWriter
...
//        WriteStateName(SubModel[i].f_fin_state, 'IniFile');
...
begin
  ...
  //    inif_fn := GM_OutPutPath + '\' + inif_fn;
      SubModel[i].SaveFinalState(SubModel[i].f_fin_state, inif_fn);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      SubModel[i].f_fin_state.Flush;
      SubModel[i].f_fin_state.Close;
//      SubModel[i].f_fin_state.FreeInstance;
//      SubModel[i].f_fin_state := NIL;
    end;
    // close(SubModel[i].f_fin_state);
  end;
end;
...
{$ENDIF}
    ...
    if self.GM_OutPutPath <> '' then
      ActIniFile.WriteString(  'FileNames', 'OutPutDir', GM_OutPutPath);
...
begin
  ...
  // if fileExists(WeatherFilefn) then
  // WeatherFile.Init(WeatherFileFN);
  if StateIniFile <> NIL then
    StateIniFile.Free
  else begin
    StateIniFile := TMyIniFile.create(StateInifilefn,  TEncoding.UTF8);
    StateIniFile.CaseSensitive := false;    if ParamInifile <> NIL then
  end;

  if ParamIniFile <> NIL then
    ParamInifile.Free else
  begin
    ParamInifile := TMyIniFile.create(ParamIniFilefn,  TEncoding.UTF8);
    ParamInifile.CaseSensitive := false;
  end;
  if OptionIniFile <> NIL then
    OptionIniFile.Free else
  begin
...
begin
 ...
 // rewrite(f);
  ...
  if f_fin_state = NIL then
     f_fin_state := TStreamWriter.Create(IniFile, false, TEncoding.UTF8, 4096);
...
//    ParStrList.Strings[i] := Param.Name;
    ...
    // removed because it ended in an run time error if a name within a sorted list
    // is changed
...
var
  ...
  i: Integer;
  value: real;
  StateIniFilename : string;
  ValueExists : boolean;
  StateIniFile : TMemIniFile;
begin
  // (Re)initialization of all state variables
  StateStrList.Sorted := false;
  for i := 0 to self.StateStrList.count - 1 do
  begin
    // create a temporary variable for the state variable
    StateIniFile := GlobMod.StateIniFile;
    State := TState(StateStrList.objects[i]);
 ...
 //   StateStrList.Strings[i] := State.Name;
    ...
    State.c := 0;
    // the value of the state variable is set to the default value
    State.v := State.DefaultValue;
    // if the state variable is selected for initialisation from file
    if State.ReadFromIniFile and (State.Name <> '') then
      // if the state ini file of the global model exists
      if (GlobMod.StateIniFile <> nil) then
      begin
//        GlobMod.StateIniFile.UpdateFile;
//        valueexists := GlobMod.StateIniFile.valueexists(submodname, State.Name);
        ValueExists := true; //StateIniFile.ValueExists(submodname, State.Name);
        if ValueExists then
        begin
//          value := GlobMod.StateIniFile.ReadFloat(submodname, State.Name, 0);
          value := StateIniFile.ReadFloat(submodname, State.Name, 0);
          State.v := value;
          State.wasreadfromfile := true;
        end // value already exists in state ini file
        else  // write a value into the inifile
        begin
          State.v := 0;
...
//              State.DefaultValue);
          if StateIniFile <> nil then
            StateIniFile.WriteFloat(submodname, State.Name,
              State.DefaultValue);
        end;
        State.iv := State.v;
      end;
